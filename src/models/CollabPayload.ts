/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface CollabPayload
 */
export interface CollabPayload {
    /**
     * Content maps a document field to its ProseMirror markup JSON.
     * @type {{ [key: string]: string; }}
     * @memberof CollabPayload
     */
    content?: { [key: string]: string; };
    /**
     * Source is the blob ref a getContent reads the snapshot from. Absent means
     * there is no snapshot to read, which answers empty content.
     * @type {string}
     * @memberof CollabPayload
     */
    source?: string;
    /**
     * Updates carries, per field, a base64 Y.js state update encoding the SAME
     * markup — the front computes it (markupToYDoc → encodeStateAsUpdate) so a
     * createContent seeds the live-editing lane's update log, not just the
     * snapshot blob. Without it a dialog-created description is invisible in the
     * collaborative editor, which replays the ydoc log, never the snapshot.
     * @type {{ [key: string]: string; }}
     * @memberof CollabPayload
     */
    updates?: { [key: string]: string; };
}

/**
 * Check if a given object implements the CollabPayload interface.
 */
export function instanceOfCollabPayload(value: object): value is CollabPayload {
    return true;
}

export function CollabPayloadFromJSON(json: any): CollabPayload {
    return CollabPayloadFromJSONTyped(json, false);
}

export function CollabPayloadFromJSONTyped(json: any, ignoreDiscriminator: boolean): CollabPayload {
    if (json == null) {
        return json;
    }
    return {
        
        'content': json['content'] == null ? undefined : json['content'],
        'source': json['source'] == null ? undefined : json['source'],
        'updates': json['updates'] == null ? undefined : json['updates'],
    };
}

export function CollabPayloadToJSON(json: any): CollabPayload {
    return CollabPayloadToJSONTyped(json, false);
}

export function CollabPayloadToJSONTyped(value?: CollabPayload | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'content': value['content'],
        'source': value['source'],
        'updates': value['updates'],
    };
}

