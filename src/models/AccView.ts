/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface AccView
 */
export interface AccView {
    /**
     * Basis is the qualification category: income, net_worth, professional_license,
     * or entity.
     * @type {string}
     * @memberof AccView
     */
    basis?: string;
    /**
     * CreatedAt is the unix second the record was created.
     * @type {number}
     * @memberof AccView
     */
    createdAt?: number;
    /**
     * EvidenceDocID references an evidence document in the org's sealed data room.
     * @type {string}
     * @memberof AccView
     */
    evidenceDocId?: string;
    /**
     * ExpiresAt is the unix second a confirmation ages out; 0 means none.
     * @type {number}
     * @memberof AccView
     */
    expiresAt?: number;
    /**
     * ID is the accreditation record's opaque id.
     * @type {string}
     * @memberof AccView
     */
    id?: string;
    /**
     * Method is how the state was established: self_attested, third_party_letter,
     * or provider_verified.
     * @type {string}
     * @memberof AccView
     */
    method?: string;
    /**
     * Note is a non-PII operator note.
     * @type {string}
     * @memberof AccView
     */
    note?: string;
    /**
     * ReviewerSub is the org user who recorded a decision on this record.
     * @type {string}
     * @memberof AccView
     */
    reviewerSub?: string;
    /**
     * Status is the tracked state: asserted, provider_verified, reviewer_confirmed,
     * rejected, or expired.
     * @type {string}
     * @memberof AccView
     */
    status?: string;
    /**
     * SubjectID is the opaque id of the subject the record is about.
     * @type {string}
     * @memberof AccView
     */
    subjectId?: string;
    /**
     * UpdatedAt is the unix second the record last changed.
     * @type {number}
     * @memberof AccView
     */
    updatedAt?: number;
}

/**
 * Check if a given object implements the AccView interface.
 */
export function instanceOfAccView(value: object): value is AccView {
    return true;
}

export function AccViewFromJSON(json: any): AccView {
    return AccViewFromJSONTyped(json, false);
}

export function AccViewFromJSONTyped(json: any, ignoreDiscriminator: boolean): AccView {
    if (json == null) {
        return json;
    }
    return {
        
        'basis': json['basis'] == null ? undefined : json['basis'],
        'createdAt': json['createdAt'] == null ? undefined : json['createdAt'],
        'evidenceDocId': json['evidenceDocId'] == null ? undefined : json['evidenceDocId'],
        'expiresAt': json['expiresAt'] == null ? undefined : json['expiresAt'],
        'id': json['id'] == null ? undefined : json['id'],
        'method': json['method'] == null ? undefined : json['method'],
        'note': json['note'] == null ? undefined : json['note'],
        'reviewerSub': json['reviewerSub'] == null ? undefined : json['reviewerSub'],
        'status': json['status'] == null ? undefined : json['status'],
        'subjectId': json['subjectId'] == null ? undefined : json['subjectId'],
        'updatedAt': json['updatedAt'] == null ? undefined : json['updatedAt'],
    };
}

export function AccViewToJSON(json: any): AccView {
    return AccViewToJSONTyped(json, false);
}

export function AccViewToJSONTyped(value?: AccView | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'basis': value['basis'],
        'createdAt': value['createdAt'],
        'evidenceDocId': value['evidenceDocId'],
        'expiresAt': value['expiresAt'],
        'id': value['id'],
        'method': value['method'],
        'note': value['note'],
        'reviewerSub': value['reviewerSub'],
        'status': value['status'],
        'subjectId': value['subjectId'],
        'updatedAt': value['updatedAt'],
    };
}

