/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface ReferenceAnswer
 */
export interface ReferenceAnswer {
    /**
     * Age is how old that is, as a duration.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    age?: string;
    /**
     * AsOf is when the oldest contributing publisher was current, RFC 3339.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    asOf?: string;
    /**
     * From is override or baseline — which plane answered.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    from?: string;
    /**
     * Hit is whether the key is a member. It is meaningful ONLY when Refusal is
     * empty: false with a refusal means the set could not be consulted, which is
     * not the same as the key being clean.
     * @type {boolean}
     * @memberof ReferenceAnswer
     */
    hit?: boolean;
    /**
     * Key is the key as asked.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    key?: string;
    /**
     * Matched is the member that covered the key, which for a domain or a network
     * is the enclosing entry rather than the key itself.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    matched?: string;
    /**
     * Refusal is why the set could not be consulted, when it could not: never
     * loaded, held elsewhere, or a source we hold no licence for. Non-empty means
     * Hit must not be read as an answer.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    refusal?: string;
    /**
     * Score is the published risk weight where the source expresses one.
     * @type {number}
     * @memberof ReferenceAnswer
     */
    score?: number;
    /**
     * Set is the set consulted.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    set?: string;
    /**
     * Stale is whether the set is past its freshness bound. A stale set still
     * answers — yesterday's list beats none — and this is how a decision knows it
     * leaned on one.
     * @type {boolean}
     * @memberof ReferenceAnswer
     */
    stale?: boolean;
    /**
     * Value is what the publisher says about the member — class, operator,
     * scheme, region.
     * @type {{ [key: string]: string; }}
     * @memberof ReferenceAnswer
     */
    value?: { [key: string]: string; };
    /**
     * Verdict is the tenant's own allow or deny, present only for an override.
     * The baseline never carries one: it states facts and leaves the decision to
     * the caller's policy.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    verdict?: string;
    /**
     * Version is the exact baseline version consulted, composed of each
     * contributing publisher and its content digest. It is what makes a decision
     * reproducible: an auditor takes this string and knows precisely what was
     * consulted.
     * @type {string}
     * @memberof ReferenceAnswer
     */
    version?: string;
}

/**
 * Check if a given object implements the ReferenceAnswer interface.
 */
export function instanceOfReferenceAnswer(value: object): value is ReferenceAnswer {
    return true;
}

export function ReferenceAnswerFromJSON(json: any): ReferenceAnswer {
    return ReferenceAnswerFromJSONTyped(json, false);
}

export function ReferenceAnswerFromJSONTyped(json: any, ignoreDiscriminator: boolean): ReferenceAnswer {
    if (json == null) {
        return json;
    }
    return {
        
        'age': json['age'] == null ? undefined : json['age'],
        'asOf': json['asOf'] == null ? undefined : json['asOf'],
        'from': json['from'] == null ? undefined : json['from'],
        'hit': json['hit'] == null ? undefined : json['hit'],
        'key': json['key'] == null ? undefined : json['key'],
        'matched': json['matched'] == null ? undefined : json['matched'],
        'refusal': json['refusal'] == null ? undefined : json['refusal'],
        'score': json['score'] == null ? undefined : json['score'],
        'set': json['set'] == null ? undefined : json['set'],
        'stale': json['stale'] == null ? undefined : json['stale'],
        'value': json['value'] == null ? undefined : json['value'],
        'verdict': json['verdict'] == null ? undefined : json['verdict'],
        'version': json['version'] == null ? undefined : json['version'],
    };
}

export function ReferenceAnswerToJSON(json: any): ReferenceAnswer {
    return ReferenceAnswerToJSONTyped(json, false);
}

export function ReferenceAnswerToJSONTyped(value?: ReferenceAnswer | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'age': value['age'],
        'asOf': value['asOf'],
        'from': value['from'],
        'hit': value['hit'],
        'key': value['key'],
        'matched': value['matched'],
        'refusal': value['refusal'],
        'score': value['score'],
        'set': value['set'],
        'stale': value['stale'],
        'value': value['value'],
        'verdict': value['verdict'],
        'version': value['version'],
    };
}

