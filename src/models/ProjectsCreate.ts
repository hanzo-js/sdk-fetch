/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
import type { ProjectsCreateRepo } from './ProjectsCreateRepo.js';
import {
    ProjectsCreateRepoFromJSON,
    ProjectsCreateRepoFromJSONTyped,
    ProjectsCreateRepoToJSON,
    ProjectsCreateRepoToJSONTyped,
} from './ProjectsCreateRepo.js';

/**
 * 
 * @export
 * @interface ProjectsCreate
 */
export interface ProjectsCreate {
    /**
     * Analytics is the opt-OUT for the wired-by-default analytics beacon: absent
     * (nil) ⇒ ON (the default); explicit false ⇒ off. A pointer so "unset" is
     * distinguishable from "false" — the only way to turn the default off.
     * @type {boolean}
     * @memberof ProjectsCreate
     */
    analytics?: boolean;
    /**
     * Description is the one-line summary, copied onto anything forked from this
     * project.
     * @type {string}
     * @memberof ProjectsCreate
     */
    description?: string;
    /**
     * Framework is a BUILD HINT from a closed set, defaulting to static. It tells CI
     * how to build a linked repo and never gates a deploy.
     * @type {string}
     * @memberof ProjectsCreate
     */
    framework?: string;
    /**
     * License is the terms that upstream work carries.
     * @type {string}
     * @memberof ProjectsCreate
     */
    license?: string;
    /**
     * Name is the project's display name and the only REQUIRED field. When slug is
     * omitted it is also what the slug is derived from.
     * @type {string}
     * @memberof ProjectsCreate
     */
    name?: string;
    /**
     * 
     * @type {ProjectsCreateRepo}
     * @memberof ProjectsCreate
     */
    repo?: ProjectsCreateRepo;
    /**
     * Slug is the handle everything else addresses this project by: the public host
     * `<slug>.hanzo.app`, the object-store key segment, and the path parameter of
     * every later call. Derived from the name when omitted. It is a hostname label,
     * so it is constrained and reserved labels such as `api` or `admin` are refused.
     * @type {string}
     * @memberof ProjectsCreate
     */
    slug?: string;
    /**
     * Upstream credits the third-party work this project was published from. It is
     * accepted from any caller: giving away credit can only cost the publisher, so
     * it needs no gate.
     * @type {string}
     * @memberof ProjectsCreate
     */
    upstream?: string;
    /**
     * Visibility is "public" (the default when absent) or "private". Publishing
     * publicly is ungated — that is the point of a community. Going PRIVATE is
     * the paid feature, so an unfunded org asking for it is refused rather than
     * silently downgraded (see resolve).
     * @type {string}
     * @memberof ProjectsCreate
     */
    visibility?: string;
}

/**
 * Check if a given object implements the ProjectsCreate interface.
 */
export function instanceOfProjectsCreate(value: object): value is ProjectsCreate {
    return true;
}

export function ProjectsCreateFromJSON(json: any): ProjectsCreate {
    return ProjectsCreateFromJSONTyped(json, false);
}

export function ProjectsCreateFromJSONTyped(json: any, ignoreDiscriminator: boolean): ProjectsCreate {
    if (json == null) {
        return json;
    }
    return {
        
        'analytics': json['analytics'] == null ? undefined : json['analytics'],
        'description': json['description'] == null ? undefined : json['description'],
        'framework': json['framework'] == null ? undefined : json['framework'],
        'license': json['license'] == null ? undefined : json['license'],
        'name': json['name'] == null ? undefined : json['name'],
        'repo': json['repo'] == null ? undefined : ProjectsCreateRepoFromJSON(json['repo']),
        'slug': json['slug'] == null ? undefined : json['slug'],
        'upstream': json['upstream'] == null ? undefined : json['upstream'],
        'visibility': json['visibility'] == null ? undefined : json['visibility'],
    };
}

export function ProjectsCreateToJSON(json: any): ProjectsCreate {
    return ProjectsCreateToJSONTyped(json, false);
}

export function ProjectsCreateToJSONTyped(value?: ProjectsCreate | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'analytics': value['analytics'],
        'description': value['description'],
        'framework': value['framework'],
        'license': value['license'],
        'name': value['name'],
        'repo': ProjectsCreateRepoToJSON(value['repo']),
        'slug': value['slug'],
        'upstream': value['upstream'],
        'visibility': value['visibility'],
    };
}

