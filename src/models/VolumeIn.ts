/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface VolumeIn
 */
export interface VolumeIn {
    /**
     * ID is the DO volume id, from the path.
     * @type {string}
     * @memberof VolumeIn
     */
    id?: string;
    /**
     * Name is the snapshot name on the snapshot action. Blank gets a deterministic
     * "<volume>-predelete-<unix>" so the undo is findable in the DO console.
     * @type {string}
     * @memberof VolumeIn
     */
    name?: string;
    /**
     * SizeGiB is the target size on the resize action. A volume only ever grows —
     * ExpandTo is the verdict that refuses a shrink, so this is not validated here.
     * @type {number}
     * @memberof VolumeIn
     */
    sizeGiB?: number;
    /**
     * Snapshot is the snapshot-first switch on DELETE. Anything other than the literal
     * "false" snapshots before destroying — the snapshot IS the undo, so waiving it is
     * deliberate and explicit.
     * @type {string}
     * @memberof VolumeIn
     */
    snapshot?: string;
}

/**
 * Check if a given object implements the VolumeIn interface.
 */
export function instanceOfVolumeIn(value: object): value is VolumeIn {
    return true;
}

export function VolumeInFromJSON(json: any): VolumeIn {
    return VolumeInFromJSONTyped(json, false);
}

export function VolumeInFromJSONTyped(json: any, ignoreDiscriminator: boolean): VolumeIn {
    if (json == null) {
        return json;
    }
    return {
        
        'id': json['id'] == null ? undefined : json['id'],
        'name': json['name'] == null ? undefined : json['name'],
        'sizeGiB': json['sizeGiB'] == null ? undefined : json['sizeGiB'],
        'snapshot': json['snapshot'] == null ? undefined : json['snapshot'],
    };
}

export function VolumeInToJSON(json: any): VolumeIn {
    return VolumeInToJSONTyped(json, false);
}

export function VolumeInToJSONTyped(value?: VolumeIn | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'id': value['id'],
        'name': value['name'],
        'sizeGiB': value['sizeGiB'],
        'snapshot': value['snapshot'],
    };
}

