/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface ScoreReq
 */
export interface ScoreReq {
    /**
     * Comment is the grader's reasoning, truncated at 2000 characters.
     * @type {string}
     * @memberof ScoreReq
     */
    comment?: string;
    /**
     * DataType is NUMERIC, CATEGORICAL or BOOLEAN. A declared rubric overrides it
     * — a caller cannot claim a type the org's rubric contradicts.
     * @type {string}
     * @memberof ScoreReq
     */
    dataType?: string;
    /**
     * ItemID attaches the score to one graded example.
     * @type {string}
     * @memberof ScoreReq
     */
    datasetItemId?: string;
    /**
     * Dataset attaches the score to one dataset.
     * @type {string}
     * @memberof ScoreReq
     */
    datasetName?: string;
    /**
     * Name is the score name. A rubric of the same name, if the org has declared
     * one, decides this score's type and the values it may take.
     * @type {string}
     * @memberof ScoreReq
     */
    name: string;
    /**
     * RunName attaches the score to one run.
     * @type {string}
     * @memberof ScoreReq
     */
    runName?: string;
    /**
     * StringValue is the label of a CATEGORICAL score, which must be one the
     * rubric allows.
     * @type {string}
     * @memberof ScoreReq
     */
    stringValue?: string;
    /**
     * TraceID attaches the score to one model call.
     * @type {string}
     * @memberof ScoreReq
     */
    traceId?: string;
    /**
     * Value is the numeric score, which must be finite: NaN and Inf are refused.
     * A BOOLEAN score takes 0 or 1.
     * @type {number}
     * @memberof ScoreReq
     */
    value?: number;
}

/**
 * Check if a given object implements the ScoreReq interface.
 */
export function instanceOfScoreReq(value: object): value is ScoreReq {
    if (!('name' in value) || value['name'] === undefined) return false;
    return true;
}

export function ScoreReqFromJSON(json: any): ScoreReq {
    return ScoreReqFromJSONTyped(json, false);
}

export function ScoreReqFromJSONTyped(json: any, ignoreDiscriminator: boolean): ScoreReq {
    if (json == null) {
        return json;
    }
    return {
        
        'comment': json['comment'] == null ? undefined : json['comment'],
        'dataType': json['dataType'] == null ? undefined : json['dataType'],
        'datasetItemId': json['datasetItemId'] == null ? undefined : json['datasetItemId'],
        'datasetName': json['datasetName'] == null ? undefined : json['datasetName'],
        'name': json['name'],
        'runName': json['runName'] == null ? undefined : json['runName'],
        'stringValue': json['stringValue'] == null ? undefined : json['stringValue'],
        'traceId': json['traceId'] == null ? undefined : json['traceId'],
        'value': json['value'] == null ? undefined : json['value'],
    };
}

export function ScoreReqToJSON(json: any): ScoreReq {
    return ScoreReqToJSONTyped(json, false);
}

export function ScoreReqToJSONTyped(value?: ScoreReq | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'comment': value['comment'],
        'dataType': value['dataType'],
        'datasetItemId': value['datasetItemId'],
        'datasetName': value['datasetName'],
        'name': value['name'],
        'runName': value['runName'],
        'stringValue': value['stringValue'],
        'traceId': value['traceId'],
        'value': value['value'],
    };
}

