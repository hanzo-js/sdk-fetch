/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface UsageWindowView
 */
export interface UsageWindowView {
    /**
     * Account is the linked provider account the window belongs to.
     * @type {string}
     * @memberof UsageWindowView
     */
    account?: string;
    /**
     * CachedInputTokens is the prompt tokens served from the provider's cache;
     * omitted when unknown.
     * @type {number}
     * @memberof UsageWindowView
     */
    cachedInputTokens?: number;
    /**
     * Confidence says how much the counters beside it mean — a meter that
     * reported only a percentage leaves them at zero, and this is how a reader
     * tells that from a true zero.
     * @type {string}
     * @memberof UsageWindowView
     */
    confidence?: string;
    /**
     * CostCents is what the window cost on the PROVIDER's own plan, in US cents.
     * It is not a Hanzo charge.
     * @type {number}
     * @memberof UsageWindowView
     */
    costCents?: number;
    /**
     * CostLimitCents is the plan's spend ceiling for the window, in US cents.
     * @type {number}
     * @memberof UsageWindowView
     */
    costLimitCents?: number;
    /**
     * Currency is the provider's currency when it is not US cents.
     * @type {string}
     * @memberof UsageWindowView
     */
    currency?: string;
    /**
     * InputTokens is prompt tokens consumed in the window; omitted when unknown.
     * @type {number}
     * @memberof UsageWindowView
     */
    inputTokens?: number;
    /**
     * Lane is the meter lane this instance belongs to, e.g. a provider's own
     * rolling-window meter.
     * @type {string}
     * @memberof UsageWindowView
     */
    lane?: string;
    /**
     * Machine is the host whose meter reported the window.
     * @type {string}
     * @memberof UsageWindowView
     */
    machine?: string;
    /**
     * OutputTokens is completion tokens produced in the window; omitted when
     * unknown.
     * @type {number}
     * @memberof UsageWindowView
     */
    outputTokens?: number;
    /**
     * Plan is the subscription plan the account is on, as the provider names it.
     * @type {string}
     * @memberof UsageWindowView
     */
    plan?: string;
    /**
     * Requests is how many requests were made in the window; omitted when the
     * meter did not report it.
     * @type {number}
     * @memberof UsageWindowView
     */
    requests?: number;
    /**
     * ResetsAt is when this window rolls over, RFC3339 UTC; omitted when unknown.
     * @type {string}
     * @memberof UsageWindowView
     */
    resetsAt?: string;
    /**
     * Synthetic marks an instance the meter inferred rather than read.
     * @type {boolean}
     * @memberof UsageWindowView
     */
    synthetic?: boolean;
    /**
     * TotalTokens is the window's total tokens; omitted when unknown.
     * @type {number}
     * @memberof UsageWindowView
     */
    totalTokens?: number;
    /**
     * UsedPct is how much of the window's allowance is consumed, 0–100.
     * @type {number}
     * @memberof UsageWindowView
     */
    usedPct?: number;
    /**
     * Window is the window class: 6h, day, week or month.
     * @type {string}
     * @memberof UsageWindowView
     */
    window?: string;
    /**
     * WindowMinutes is the window's real length in minutes when the meter
     * reported one; omitted when it did not.
     * @type {number}
     * @memberof UsageWindowView
     */
    windowMinutes?: number;
    /**
     * WindowStart is when this window opened, RFC3339 UTC; omitted when unknown.
     * @type {string}
     * @memberof UsageWindowView
     */
    windowStart?: string;
}

/**
 * Check if a given object implements the UsageWindowView interface.
 */
export function instanceOfUsageWindowView(value: object): value is UsageWindowView {
    return true;
}

export function UsageWindowViewFromJSON(json: any): UsageWindowView {
    return UsageWindowViewFromJSONTyped(json, false);
}

export function UsageWindowViewFromJSONTyped(json: any, ignoreDiscriminator: boolean): UsageWindowView {
    if (json == null) {
        return json;
    }
    return {
        
        'account': json['account'] == null ? undefined : json['account'],
        'cachedInputTokens': json['cachedInputTokens'] == null ? undefined : json['cachedInputTokens'],
        'confidence': json['confidence'] == null ? undefined : json['confidence'],
        'costCents': json['costCents'] == null ? undefined : json['costCents'],
        'costLimitCents': json['costLimitCents'] == null ? undefined : json['costLimitCents'],
        'currency': json['currency'] == null ? undefined : json['currency'],
        'inputTokens': json['inputTokens'] == null ? undefined : json['inputTokens'],
        'lane': json['lane'] == null ? undefined : json['lane'],
        'machine': json['machine'] == null ? undefined : json['machine'],
        'outputTokens': json['outputTokens'] == null ? undefined : json['outputTokens'],
        'plan': json['plan'] == null ? undefined : json['plan'],
        'requests': json['requests'] == null ? undefined : json['requests'],
        'resetsAt': json['resetsAt'] == null ? undefined : json['resetsAt'],
        'synthetic': json['synthetic'] == null ? undefined : json['synthetic'],
        'totalTokens': json['totalTokens'] == null ? undefined : json['totalTokens'],
        'usedPct': json['usedPct'] == null ? undefined : json['usedPct'],
        'window': json['window'] == null ? undefined : json['window'],
        'windowMinutes': json['windowMinutes'] == null ? undefined : json['windowMinutes'],
        'windowStart': json['windowStart'] == null ? undefined : json['windowStart'],
    };
}

export function UsageWindowViewToJSON(json: any): UsageWindowView {
    return UsageWindowViewToJSONTyped(json, false);
}

export function UsageWindowViewToJSONTyped(value?: UsageWindowView | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'account': value['account'],
        'cachedInputTokens': value['cachedInputTokens'],
        'confidence': value['confidence'],
        'costCents': value['costCents'],
        'costLimitCents': value['costLimitCents'],
        'currency': value['currency'],
        'inputTokens': value['inputTokens'],
        'lane': value['lane'],
        'machine': value['machine'],
        'outputTokens': value['outputTokens'],
        'plan': value['plan'],
        'requests': value['requests'],
        'resetsAt': value['resetsAt'],
        'synthetic': value['synthetic'],
        'totalTokens': value['totalTokens'],
        'usedPct': value['usedPct'],
        'window': value['window'],
        'windowMinutes': value['windowMinutes'],
        'windowStart': value['windowStart'],
    };
}

