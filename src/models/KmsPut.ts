/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface KmsPut
 */
export interface KmsPut {
    /**
     * Env is the environment to write under. REQUIRED, with no default: it is
     * part of the storage key, so a silently defaulted write lands in a bucket
     * the readers that resolve project, environment and path never look in, and
     * the stale value keeps being served.
     * @type {string}
     * @memberof KmsPut
     */
    env?: string;
    /**
     * Name is the secret's name. Required.
     * @type {string}
     * @memberof KmsPut
     */
    name?: string;
    /**
     * Path is an optional subpath beneath the org root, e.g. "/ci".
     * @type {string}
     * @memberof KmsPut
     */
    path?: string;
    /**
     * Value is the secret itself. It is sealed under a fresh per-secret data key
     * before storage, so plaintext never reaches disk, and it is never echoed
     * back, logged, or carried in an error.
     * @type {string}
     * @memberof KmsPut
     */
    value?: string;
}

/**
 * Check if a given object implements the KmsPut interface.
 */
export function instanceOfKmsPut(value: object): value is KmsPut {
    return true;
}

export function KmsPutFromJSON(json: any): KmsPut {
    return KmsPutFromJSONTyped(json, false);
}

export function KmsPutFromJSONTyped(json: any, ignoreDiscriminator: boolean): KmsPut {
    if (json == null) {
        return json;
    }
    return {
        
        'env': json['env'] == null ? undefined : json['env'],
        'name': json['name'] == null ? undefined : json['name'],
        'path': json['path'] == null ? undefined : json['path'],
        'value': json['value'] == null ? undefined : json['value'],
    };
}

export function KmsPutToJSON(json: any): KmsPut {
    return KmsPutToJSONTyped(json, false);
}

export function KmsPutToJSONTyped(value?: KmsPut | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'env': value['env'],
        'name': value['name'],
        'path': value['path'],
        'value': value['value'],
    };
}

