/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface KeyTypeIn
 */
export interface KeyTypeIn {
    /**
     * Limit narrows what the minted key may reach, as `kind:name` entries:
     * `model:zen5`, `project:acme`, `product:commerce`, or `model:*` for a whole
     * kind. It only ever NARROWS — a key can never reach further than the person
     * who minted it — so an unrecognised kind costs availability, never privilege.
     * 
     * Omitted mints an unrestricted key, because that is what every key in the
     * estate is today and a default that restricted would revoke all of them.
     * 
     * Example: {"type": "secret", "limit": ["model:zen5", "project:acme"]}
     * @type {Array<string>}
     * @memberof KeyTypeIn
     */
    limit?: Array<string>;
    /**
     * Type is the key class to act on: "secret" (sk-, session-equivalent, belongs
     * on a server) or "publishable" (pk-, org-identifying, safe in a browser
     * bundle). Omitted means secret, which is what every existing caller means.
     * @type {string}
     * @memberof KeyTypeIn
     */
    type?: string;
}

/**
 * Check if a given object implements the KeyTypeIn interface.
 */
export function instanceOfKeyTypeIn(value: object): value is KeyTypeIn {
    return true;
}

export function KeyTypeInFromJSON(json: any): KeyTypeIn {
    return KeyTypeInFromJSONTyped(json, false);
}

export function KeyTypeInFromJSONTyped(json: any, ignoreDiscriminator: boolean): KeyTypeIn {
    if (json == null) {
        return json;
    }
    return {
        
        'limit': json['limit'] == null ? undefined : json['limit'],
        'type': json['type'] == null ? undefined : json['type'],
    };
}

export function KeyTypeInToJSON(json: any): KeyTypeIn {
    return KeyTypeInToJSONTyped(json, false);
}

export function KeyTypeInToJSONTyped(value?: KeyTypeIn | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'limit': value['limit'],
        'type': value['type'],
    };
}

