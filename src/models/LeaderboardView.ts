/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
import type { LeaderboardRow } from './LeaderboardRow.js';
import {
    LeaderboardRowFromJSON,
    LeaderboardRowFromJSONTyped,
    LeaderboardRowToJSON,
    LeaderboardRowToJSONTyped,
} from './LeaderboardRow.js';
import type { SelfRank } from './SelfRank.js';
import {
    SelfRankFromJSON,
    SelfRankFromJSONTyped,
    SelfRankToJSON,
    SelfRankToJSONTyped,
} from './SelfRank.js';

/**
 * 
 * @export
 * @interface LeaderboardView
 */
export interface LeaderboardView {
    /**
     * Available is false when the usage warehouse is not connected or its rollup is not
     * ready. Rows is then empty because nothing could be read — not because nobody used
     * anything. Show that difference; never render an unavailable board as a real one.
     * @type {boolean}
     * @memberof LeaderboardView
     */
    available?: boolean;
    /**
     * End is the EXCLUSIVE upper bound of the window, "2006-01-02" — the day after the
     * last one counted. A board through today reports tomorrow's date here.
     * @type {string}
     * @memberof LeaderboardView
     */
    end?: string;
    /**
     * Metric echoes the value ranked: tokens|requests|cost.
     * @type {string}
     * @memberof LeaderboardView
     */
    metric?: string;
    /**
     * Period is the window's canonical label: day|week|month|all. The server resolves
     * aliases (7d, 30d, today, …) to these, so this may differ from what was sent.
     * @type {string}
     * @memberof LeaderboardView
     */
    period?: string;
    /**
     * Rows are the ranked subjects, best first, at most the requested limit of them.
     * Always a list, never null: an empty one means nothing was read, not an error.
     * @type {Array<LeaderboardRow>}
     * @memberof LeaderboardView
     */
    rows?: Array<LeaderboardRow>;
    /**
     * Scope echoes the board that was served: personal|org|global.
     * @type {string}
     * @memberof LeaderboardView
     */
    scope?: string;
    /**
     * Self is the caller's own standing, reported even when they fall outside Rows.
     * Absent when the caller's ledger identity cannot be resolved, or when the query
     * behind it failed — never faked to keep the shape tidy.
     * @type {SelfRank}
     * @memberof LeaderboardView
     */
    self?: SelfRank;
    /**
     * Source names the table these numbers were aggregated from (the derived daily
     * rollup, hanzo.usage_rollup_daily), so an operator can tell exactly what was read.
     * @type {string}
     * @memberof LeaderboardView
     */
    source?: string;
    /**
     * Start is the first day counted, "2006-01-02" inclusive. Empty for period=all,
     * which has no lower bound at all.
     * @type {string}
     * @memberof LeaderboardView
     */
    start?: string;
    /**
     * Subject is what the rows stand for — "user" on a personal or org board, "org" on
     * the global one. It tells a client whether Handle names a person or a company.
     * @type {string}
     * @memberof LeaderboardView
     */
    subject?: string;
    /**
     * Total is how many subjects were ranked in the window — the org's active users, or
     * the active/opted-in orgs on the global board. It is the universe the ranks are
     * out of, so it is normally larger than len(rows).
     * @type {number}
     * @memberof LeaderboardView
     */
    total?: number;
}

/**
 * Check if a given object implements the LeaderboardView interface.
 */
export function instanceOfLeaderboardView(value: object): value is LeaderboardView {
    return true;
}

export function LeaderboardViewFromJSON(json: any): LeaderboardView {
    return LeaderboardViewFromJSONTyped(json, false);
}

export function LeaderboardViewFromJSONTyped(json: any, ignoreDiscriminator: boolean): LeaderboardView {
    if (json == null) {
        return json;
    }
    return {
        
        'available': json['available'] == null ? undefined : json['available'],
        'end': json['end'] == null ? undefined : json['end'],
        'metric': json['metric'] == null ? undefined : json['metric'],
        'period': json['period'] == null ? undefined : json['period'],
        'rows': json['rows'] == null ? undefined : ((json['rows'] as Array<any>).map(LeaderboardRowFromJSON)),
        'scope': json['scope'] == null ? undefined : json['scope'],
        'self': json['self'] == null ? undefined : SelfRankFromJSON(json['self']),
        'source': json['source'] == null ? undefined : json['source'],
        'start': json['start'] == null ? undefined : json['start'],
        'subject': json['subject'] == null ? undefined : json['subject'],
        'total': json['total'] == null ? undefined : json['total'],
    };
}

export function LeaderboardViewToJSON(json: any): LeaderboardView {
    return LeaderboardViewToJSONTyped(json, false);
}

export function LeaderboardViewToJSONTyped(value?: LeaderboardView | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'available': value['available'],
        'end': value['end'],
        'metric': value['metric'],
        'period': value['period'],
        'rows': value['rows'] == null ? undefined : ((value['rows'] as Array<any>).map(LeaderboardRowToJSON)),
        'scope': value['scope'],
        'self': SelfRankToJSON(value['self']),
        'source': value['source'],
        'start': value['start'],
        'subject': value['subject'],
        'total': value['total'],
    };
}

