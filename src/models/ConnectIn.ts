/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface ConnectIn
 */
export interface ConnectIn {
    /**
     * AccountID is the provider account the credential should be scoped to, for the
     * providers whose Verify needs one (Cloudflare). Ignored by the OAuth path.
     * @type {string}
     * @memberof ConnectIn
     */
    accountId?: string;
    /**
     * Provider is the connector's registry id, from the :provider path segment.
     * @type {string}
     * @memberof ConnectIn
     */
    provider?: string;
    /**
     * Token is the customer's provider credential. Its PRESENCE — not its value —
     * is what selects the apikey seal over the OAuth flow for a provider that
     * offers both: {"token":"…"}, even empty, is an apikey attempt (→ verify, which
     * answers the "token required" 400 on an empty value), while a body with no
     * token key (the console Connect button, `hanzo connector add` with no --token)
     * starts OAuth. Read on STDIN by the CLI, never argv; never logged or echoed.
     * @type {string}
     * @memberof ConnectIn
     */
    token?: string;
}

/**
 * Check if a given object implements the ConnectIn interface.
 */
export function instanceOfConnectIn(value: object): value is ConnectIn {
    return true;
}

export function ConnectInFromJSON(json: any): ConnectIn {
    return ConnectInFromJSONTyped(json, false);
}

export function ConnectInFromJSONTyped(json: any, ignoreDiscriminator: boolean): ConnectIn {
    if (json == null) {
        return json;
    }
    return {
        
        'accountId': json['accountId'] == null ? undefined : json['accountId'],
        'provider': json['provider'] == null ? undefined : json['provider'],
        'token': json['token'] == null ? undefined : json['token'],
    };
}

export function ConnectInToJSON(json: any): ConnectIn {
    return ConnectInToJSONTyped(json, false);
}

export function ConnectInToJSONTyped(value?: ConnectIn | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'accountId': value['accountId'],
        'provider': value['provider'],
        'token': value['token'],
    };
}

