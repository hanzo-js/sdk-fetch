/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface PatchSessionIn
 */
export interface PatchSessionIn {
    /**
     * Cwd is where the session is working NOW.
     * 
     * It was write-once — captured at register and never again — which is right
     * for a run that starts in a directory and stays there, and wrong for a linked
     * shell, which is a place a person moves around in. The console showed the
     * directory `hanzo link` happened to be run from and kept showing it after the
     * shell had walked away, so the field answered "which work is this" with an
     * answer that was true once. A pointer, so an unchanged path is an omitted
     * field rather than a repeated write.
     * @type {string}
     * @memberof PatchSessionIn
     */
    cwd?: string;
    /**
     * ID is the session to update, from the path.
     * @type {string}
     * @memberof PatchSessionIn
     */
    id?: string;
    /**
     * Project tags the product this session built; Published is the author's
     * decision to let anyone read the story (provenance.go). Both are pointers so
     * "absent" and "cleared" are different requests.
     * @type {string}
     * @memberof PatchSessionIn
     */
    project?: string;
    /**
     * 
     * @type {boolean}
     * @memberof PatchSessionIn
     */
    published?: boolean;
    /**
     * 
     * @type {string}
     * @memberof PatchSessionIn
     */
    status?: string;
    /**
     * Target re-dispatches a session to a run-target (the #48 association). "" detaches.
     * @type {string}
     * @memberof PatchSessionIn
     */
    target?: string;
    /**
     * Terminal publishes (or, with "", withdraws) the URL this session's live
     * terminal can be watched at. A pointer so "absent" and "withdrawn" are
     * different requests: a session that stops sharing must be able to say so.
     * @type {string}
     * @memberof PatchSessionIn
     */
    terminal?: string;
    /**
     * 
     * @type {string}
     * @memberof PatchSessionIn
     */
    title?: string;
}

/**
 * Check if a given object implements the PatchSessionIn interface.
 */
export function instanceOfPatchSessionIn(value: object): value is PatchSessionIn {
    return true;
}

export function PatchSessionInFromJSON(json: any): PatchSessionIn {
    return PatchSessionInFromJSONTyped(json, false);
}

export function PatchSessionInFromJSONTyped(json: any, ignoreDiscriminator: boolean): PatchSessionIn {
    if (json == null) {
        return json;
    }
    return {
        
        'cwd': json['cwd'] == null ? undefined : json['cwd'],
        'id': json['id'] == null ? undefined : json['id'],
        'project': json['project'] == null ? undefined : json['project'],
        'published': json['published'] == null ? undefined : json['published'],
        'status': json['status'] == null ? undefined : json['status'],
        'target': json['target'] == null ? undefined : json['target'],
        'terminal': json['terminal'] == null ? undefined : json['terminal'],
        'title': json['title'] == null ? undefined : json['title'],
    };
}

export function PatchSessionInToJSON(json: any): PatchSessionIn {
    return PatchSessionInToJSONTyped(json, false);
}

export function PatchSessionInToJSONTyped(value?: PatchSessionIn | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'cwd': value['cwd'],
        'id': value['id'],
        'project': value['project'],
        'published': value['published'],
        'status': value['status'],
        'target': value['target'],
        'terminal': value['terminal'],
        'title': value['title'],
    };
}

