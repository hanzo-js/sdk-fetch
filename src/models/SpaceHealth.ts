/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface SpaceHealth
 */
export interface SpaceHealth {
    /**
     * Error is why the probe is degraded, in plain words. Absent when it is not.
     * @type {string}
     * @memberof SpaceHealth
     */
    error?: string;
    /**
     * Presign is whether upload and download URLs can be minted, which needs a
     * PUBLIC endpoint on top of the credentials. False does not make the surface
     * degraded — listing spaces, drives and folders still works, only the bytes
     * cannot be reached.
     * @type {boolean}
     * @memberof SpaceHealth
     */
    presign?: boolean;
    /**
     * Ready is whether this deployment can serve drive and file operations at all:
     * true only when object-store credentials are configured.
     * @type {boolean}
     * @memberof SpaceHealth
     */
    ready?: boolean;
    /**
     * Service names the subsystem this probe is for. Always "space".
     * @type {string}
     * @memberof SpaceHealth
     */
    service?: string;
    /**
     * Status is "ok" when the store is reachable in principle, "degraded" when it
     * is not. It is the field to read; the HTTP status carries the same fact for a
     * caller that only looks at the code.
     * @type {string}
     * @memberof SpaceHealth
     */
    status?: string;
}

/**
 * Check if a given object implements the SpaceHealth interface.
 */
export function instanceOfSpaceHealth(value: object): value is SpaceHealth {
    return true;
}

export function SpaceHealthFromJSON(json: any): SpaceHealth {
    return SpaceHealthFromJSONTyped(json, false);
}

export function SpaceHealthFromJSONTyped(json: any, ignoreDiscriminator: boolean): SpaceHealth {
    if (json == null) {
        return json;
    }
    return {
        
        'error': json['error'] == null ? undefined : json['error'],
        'presign': json['presign'] == null ? undefined : json['presign'],
        'ready': json['ready'] == null ? undefined : json['ready'],
        'service': json['service'] == null ? undefined : json['service'],
        'status': json['status'] == null ? undefined : json['status'],
    };
}

export function SpaceHealthToJSON(json: any): SpaceHealth {
    return SpaceHealthToJSONTyped(json, false);
}

export function SpaceHealthToJSONTyped(value?: SpaceHealth | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'error': value['error'],
        'presign': value['presign'],
        'ready': value['ready'],
        'service': value['service'],
        'status': value['status'],
    };
}

