/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface ScoreView
 */
export interface ScoreView {
    /**
     * Comment is the grader's reasoning, truncated at 2000 characters.
     * @type {string}
     * @memberof ScoreView
     */
    comment?: string;
    /**
     * DataType is NUMERIC, CATEGORICAL or BOOLEAN.
     * @type {string}
     * @memberof ScoreView
     */
    dataType?: string;
    /**
     * ID is the score event's handle.
     * @type {string}
     * @memberof ScoreView
     */
    id?: string;
    /**
     * Name is the score name, which a rubric of the same name governs.
     * @type {string}
     * @memberof ScoreView
     */
    name?: string;
    /**
     * RunName is the run this score was recorded under, when it came from one.
     * @type {string}
     * @memberof ScoreView
     */
    runName?: string;
    /**
     * StringValue is the label of a CATEGORICAL score.
     * @type {string}
     * @memberof ScoreView
     */
    stringValue?: string;
    /**
     * Timestamp is when the score was recorded.
     * @type {string}
     * @memberof ScoreView
     */
    timestamp?: string;
    /**
     * TraceID is the model call this score grades, when it grades one.
     * @type {string}
     * @memberof ScoreView
     */
    traceId?: string;
    /**
     * Value is the numeric score; for BOOLEAN it is 0 or 1.
     * @type {number}
     * @memberof ScoreView
     */
    value?: number;
}

/**
 * Check if a given object implements the ScoreView interface.
 */
export function instanceOfScoreView(value: object): value is ScoreView {
    return true;
}

export function ScoreViewFromJSON(json: any): ScoreView {
    return ScoreViewFromJSONTyped(json, false);
}

export function ScoreViewFromJSONTyped(json: any, ignoreDiscriminator: boolean): ScoreView {
    if (json == null) {
        return json;
    }
    return {
        
        'comment': json['comment'] == null ? undefined : json['comment'],
        'dataType': json['dataType'] == null ? undefined : json['dataType'],
        'id': json['id'] == null ? undefined : json['id'],
        'name': json['name'] == null ? undefined : json['name'],
        'runName': json['runName'] == null ? undefined : json['runName'],
        'stringValue': json['stringValue'] == null ? undefined : json['stringValue'],
        'timestamp': json['timestamp'] == null ? undefined : json['timestamp'],
        'traceId': json['traceId'] == null ? undefined : json['traceId'],
        'value': json['value'] == null ? undefined : json['value'],
    };
}

export function ScoreViewToJSON(json: any): ScoreView {
    return ScoreViewToJSONTyped(json, false);
}

export function ScoreViewToJSONTyped(value?: ScoreView | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'comment': value['comment'],
        'dataType': value['dataType'],
        'id': value['id'],
        'name': value['name'],
        'runName': value['runName'],
        'stringValue': value['stringValue'],
        'timestamp': value['timestamp'],
        'traceId': value['traceId'],
        'value': value['value'],
    };
}

