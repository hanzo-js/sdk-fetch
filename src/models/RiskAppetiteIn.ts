/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface RiskAppetiteIn
 */
export interface RiskAppetiteIn {
    /**
     * Live turns the model out of shadow. It defaults to FALSE on every call, so
     * going live is always an explicit act and never a side effect of changing a
     * number.
     * 
     * Setting it requires an ADMIN of this organisation. Arming decides whether the
     * model may change an outcome at all — a payment frozen, a grant refused — for
     * every customer this organisation has, which is a governance act rather than a
     * tuning one. Stating the appetite and the sample needs no admin.
     * @type {boolean}
     * @memberof RiskAppetiteIn
     */
    live?: boolean;
    /**
     * Review is the share of the stream that may be sent for examination, in
     * (0, 0.5]. The alert threshold is derived from it as a quantile of the scores
     * actually observed, so the level is governed rather than tuned.
     * @type {number}
     * @memberof RiskAppetiteIn
     */
    review?: number;
    /**
     * Sample is the share of below-the-line events retained for review, in
     * [0, 1]. It is the instrument that measures what the model missed; there are
     * no labels, so nothing else can.
     * @type {number}
     * @memberof RiskAppetiteIn
     */
    sample?: number;
}

/**
 * Check if a given object implements the RiskAppetiteIn interface.
 */
export function instanceOfRiskAppetiteIn(value: object): value is RiskAppetiteIn {
    return true;
}

export function RiskAppetiteInFromJSON(json: any): RiskAppetiteIn {
    return RiskAppetiteInFromJSONTyped(json, false);
}

export function RiskAppetiteInFromJSONTyped(json: any, ignoreDiscriminator: boolean): RiskAppetiteIn {
    if (json == null) {
        return json;
    }
    return {
        
        'live': json['live'] == null ? undefined : json['live'],
        'review': json['review'] == null ? undefined : json['review'],
        'sample': json['sample'] == null ? undefined : json['sample'],
    };
}

export function RiskAppetiteInToJSON(json: any): RiskAppetiteIn {
    return RiskAppetiteInToJSONTyped(json, false);
}

export function RiskAppetiteInToJSONTyped(value?: RiskAppetiteIn | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'live': value['live'],
        'review': value['review'],
        'sample': value['sample'],
    };
}

