/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface EsignRecipient
 */
export interface EsignRecipient {
    /**
     * Email is where this recipient's signing link is meant to go, lower-cased.
     * @type {string}
     * @memberof EsignRecipient
     */
    email?: string;
    /**
     * ID is the recipient id, which is what a field is placed against.
     * @type {string}
     * @memberof EsignRecipient
     */
    id?: string;
    /**
     * Name is the recipient's display name, empty when none was given.
     * @type {string}
     * @memberof EsignRecipient
     */
    name?: string;
    /**
     * ReadStatus is NOT_OPENED until they first open their link, then OPENED.
     * @type {string}
     * @memberof EsignRecipient
     */
    readStatus?: string;
    /**
     * RejectionReason is why they declined, null unless they did.
     * @type {string}
     * @memberof EsignRecipient
     */
    rejectionReason?: string;
    /**
     * Role is SIGNER, CC, VIEWER, APPROVER or ASSISTANT. A document waits only for
     * its SIGNERs and APPROVERs before it can seal.
     * @type {string}
     * @memberof EsignRecipient
     */
    role?: string;
    /**
     * SendStatus is NOT_SENT until the document goes out, then SENT. A CC recipient
     * is SENT from the moment they are added.
     * @type {string}
     * @memberof EsignRecipient
     */
    sendStatus?: string;
    /**
     * SignedAt is when they finished or declined, in unix milliseconds; null while
     * neither has happened.
     * @type {number}
     * @memberof EsignRecipient
     */
    signedAt?: number;
    /**
     * SigningOrder is their position in a SEQUENTIAL document, null when they were
     * added without one. A PARALLEL document ignores it.
     * @type {number}
     * @memberof EsignRecipient
     */
    signingOrder?: number;
    /**
     * SigningStatus is NOT_SIGNED, SIGNED or REJECTED. A CC recipient is SIGNED
     * from the moment they are added, because they are never asked.
     * @type {string}
     * @memberof EsignRecipient
     */
    signingStatus?: string;
}

/**
 * Check if a given object implements the EsignRecipient interface.
 */
export function instanceOfEsignRecipient(value: object): value is EsignRecipient {
    return true;
}

export function EsignRecipientFromJSON(json: any): EsignRecipient {
    return EsignRecipientFromJSONTyped(json, false);
}

export function EsignRecipientFromJSONTyped(json: any, ignoreDiscriminator: boolean): EsignRecipient {
    if (json == null) {
        return json;
    }
    return {
        
        'email': json['email'] == null ? undefined : json['email'],
        'id': json['id'] == null ? undefined : json['id'],
        'name': json['name'] == null ? undefined : json['name'],
        'readStatus': json['readStatus'] == null ? undefined : json['readStatus'],
        'rejectionReason': json['rejectionReason'] == null ? undefined : json['rejectionReason'],
        'role': json['role'] == null ? undefined : json['role'],
        'sendStatus': json['sendStatus'] == null ? undefined : json['sendStatus'],
        'signedAt': json['signedAt'] == null ? undefined : json['signedAt'],
        'signingOrder': json['signingOrder'] == null ? undefined : json['signingOrder'],
        'signingStatus': json['signingStatus'] == null ? undefined : json['signingStatus'],
    };
}

export function EsignRecipientToJSON(json: any): EsignRecipient {
    return EsignRecipientToJSONTyped(json, false);
}

export function EsignRecipientToJSONTyped(value?: EsignRecipient | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'email': value['email'],
        'id': value['id'],
        'name': value['name'],
        'readStatus': value['readStatus'],
        'rejectionReason': value['rejectionReason'],
        'role': value['role'],
        'sendStatus': value['sendStatus'],
        'signedAt': value['signedAt'],
        'signingOrder': value['signingOrder'],
        'signingStatus': value['signingStatus'],
    };
}

