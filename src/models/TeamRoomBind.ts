/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface TeamRoomBind
 */
export interface TeamRoomBind {
    /**
     * Bindings REPLACES what the room is about, wholly. It is a replace and
     * not a merge because a caller that cannot remove a binding would have no way
     * to correct a wrong one, and an empty list sent explicitly is how a room
     * is unbound. Absent (null) leaves the existing list alone.
     * @type {Array<string>}
     * @memberof TeamRoomBind
     */
    bindings?: Array<string>;
    /**
     * ID is the room to bind, from the path. The URL is the authority; a body
     * carrying another id cannot redirect the write.
     * @type {string}
     * @memberof TeamRoomBind
     */
    id?: string;
    /**
     * Life sets the lifecycle intent: "standing" or "bound". Any other
     * value is refused rather than stored, so a reader never has to interpret a
     * third one. Empty leaves the current intent unchanged.
     * @type {string}
     * @memberof TeamRoomBind
     */
    life?: string;
    /**
     * Space names the space holding the room. It is required, because
     * a room id is unique only within one and searching every space for a
     * matching id would make the write's target depend on iteration order.
     * @type {string}
     * @memberof TeamRoomBind
     */
    space?: string;
}

/**
 * Check if a given object implements the TeamRoomBind interface.
 */
export function instanceOfTeamRoomBind(value: object): value is TeamRoomBind {
    return true;
}

export function TeamRoomBindFromJSON(json: any): TeamRoomBind {
    return TeamRoomBindFromJSONTyped(json, false);
}

export function TeamRoomBindFromJSONTyped(json: any, ignoreDiscriminator: boolean): TeamRoomBind {
    if (json == null) {
        return json;
    }
    return {
        
        'bindings': json['bindings'] == null ? undefined : json['bindings'],
        'id': json['id'] == null ? undefined : json['id'],
        'life': json['life'] == null ? undefined : json['life'],
        'space': json['space'] == null ? undefined : json['space'],
    };
}

export function TeamRoomBindToJSON(json: any): TeamRoomBind {
    return TeamRoomBindToJSONTyped(json, false);
}

export function TeamRoomBindToJSONTyped(value?: TeamRoomBind | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'bindings': value['bindings'],
        'id': value['id'],
        'life': value['life'],
        'space': value['space'],
    };
}

