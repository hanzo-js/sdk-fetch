/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface LinkView
 */
export interface LinkView {
    /**
     * Account is the provider-side account identifier, when the collector knows it.
     * @type {string}
     * @memberof LinkView
     */
    account?: string;
    /**
     * Billing is how this account's inference bills — plan (the user's own
     * subscription, metered here for visibility only) or commerce (the gateway
     * path). Derived from Kind, never stored.
     * @type {string}
     * @memberof LinkView
     */
    billing?: string;
    /**
     * CreatedAt is when the link was first registered, RFC 3339 UTC.
     * @type {string}
     * @memberof LinkView
     */
    createdAt?: string;
    /**
     * Host is the machine's human hostname label, from its most recent report.
     * @type {string}
     * @memberof LinkView
     */
    host?: string;
    /**
     * ID is the link's opaque handle ("link_" + 32 hex chars).
     * @type {string}
     * @memberof LinkView
     */
    id?: string;
    /**
     * Kind is how the account authenticates: subscription or apikey.
     * @type {string}
     * @memberof LinkView
     */
    kind?: string;
    /**
     * LastSeen is when the account last reported, RFC 3339 UTC.
     * @type {string}
     * @memberof LinkView
     */
    lastSeen?: string;
    /**
     * Machine is the stable machine identifier the collector reports.
     * @type {string}
     * @memberof LinkView
     */
    machine?: string;
    /**
     * OS is the machine's operating system label.
     * @type {string}
     * @memberof LinkView
     */
    os?: string;
    /**
     * Plan is the provider plan label (e.g. "Claude Max").
     * @type {string}
     * @memberof LinkView
     */
    plan?: string;
    /**
     * Provider is the AI provider this account belongs to (claude, openai, hanzo…).
     * @type {string}
     * @memberof LinkView
     */
    provider?: string;
    /**
     * Status is linked or revoked. Revoked rows are retained for history.
     * @type {string}
     * @memberof LinkView
     */
    status?: string;
    /**
     * UpdatedAt is when the link was last refreshed, RFC 3339 UTC.
     * @type {string}
     * @memberof LinkView
     */
    updatedAt?: string;
    /**
     * 
     * @type {any}
     * @memberof LinkView
     */
    usage?: any | null;
    /**
     * User is the owning subject — the validated caller who registered the link.
     * @type {string}
     * @memberof LinkView
     */
    user?: string;
}

/**
 * Check if a given object implements the LinkView interface.
 */
export function instanceOfLinkView(value: object): value is LinkView {
    return true;
}

export function LinkViewFromJSON(json: any): LinkView {
    return LinkViewFromJSONTyped(json, false);
}

export function LinkViewFromJSONTyped(json: any, ignoreDiscriminator: boolean): LinkView {
    if (json == null) {
        return json;
    }
    return {
        
        'account': json['account'] == null ? undefined : json['account'],
        'billing': json['billing'] == null ? undefined : json['billing'],
        'createdAt': json['createdAt'] == null ? undefined : json['createdAt'],
        'host': json['host'] == null ? undefined : json['host'],
        'id': json['id'] == null ? undefined : json['id'],
        'kind': json['kind'] == null ? undefined : json['kind'],
        'lastSeen': json['lastSeen'] == null ? undefined : json['lastSeen'],
        'machine': json['machine'] == null ? undefined : json['machine'],
        'os': json['os'] == null ? undefined : json['os'],
        'plan': json['plan'] == null ? undefined : json['plan'],
        'provider': json['provider'] == null ? undefined : json['provider'],
        'status': json['status'] == null ? undefined : json['status'],
        'updatedAt': json['updatedAt'] == null ? undefined : json['updatedAt'],
        'usage': json['usage'] == null ? undefined : json['usage'],
        'user': json['user'] == null ? undefined : json['user'],
    };
}

export function LinkViewToJSON(json: any): LinkView {
    return LinkViewToJSONTyped(json, false);
}

export function LinkViewToJSONTyped(value?: LinkView | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'account': value['account'],
        'billing': value['billing'],
        'createdAt': value['createdAt'],
        'host': value['host'],
        'id': value['id'],
        'kind': value['kind'],
        'lastSeen': value['lastSeen'],
        'machine': value['machine'],
        'os': value['os'],
        'plan': value['plan'],
        'provider': value['provider'],
        'status': value['status'],
        'updatedAt': value['updatedAt'],
        'usage': value['usage'],
        'user': value['user'],
    };
}

