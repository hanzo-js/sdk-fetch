/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface DocRow
 */
export interface DocRow {
    /**
     * Attested reports whether somebody OUTSIDE this organization put their name
     * to it. Those are the artifacts a reviewer asks for, and they are released
     * through a grant rather than published — there is no field that can say
     * otherwise.
     * @type {boolean}
     * @memberof DocRow
     */
    attested?: boolean;
    /**
     * Href is where to read it, present only when this reader may.
     * @type {string}
     * @memberof DocRow
     */
    href?: string;
    /**
     * ID is the document's id within this organization's centre.
     * @type {string}
     * @memberof DocRow
     */
    id?: string;
    /**
     * Kind is the artifact type — soc2, iso, pentest, letter, caiq, sig, vsa,
     * questionnaire, policy or other.
     * @type {string}
     * @memberof DocRow
     */
    kind?: string;
    /**
     * Label is the artifact type in words, for rendering.
     * @type {string}
     * @memberof DocRow
     */
    label?: string;
    /**
     * Note is anything the organization says about this artifact.
     * @type {string}
     * @memberof DocRow
     */
    note?: string;
    /**
     * Released reports whether THIS reader may read it. False means the artifact
     * exists and is available on request.
     * @type {boolean}
     * @memberof DocRow
     */
    released?: boolean;
    /**
     * Tier is "public" or "gated". It defaults to gated, so a new artifact is
     * closed until somebody opens it deliberately.
     * @type {string}
     * @memberof DocRow
     */
    tier?: string;
    /**
     * Title is what the document is called.
     * @type {string}
     * @memberof DocRow
     */
    title?: string;
    /**
     * Updated is when the record last changed, unix milliseconds.
     * @type {number}
     * @memberof DocRow
     */
    updated?: number;
}

/**
 * Check if a given object implements the DocRow interface.
 */
export function instanceOfDocRow(value: object): value is DocRow {
    return true;
}

export function DocRowFromJSON(json: any): DocRow {
    return DocRowFromJSONTyped(json, false);
}

export function DocRowFromJSONTyped(json: any, ignoreDiscriminator: boolean): DocRow {
    if (json == null) {
        return json;
    }
    return {
        
        'attested': json['attested'] == null ? undefined : json['attested'],
        'href': json['href'] == null ? undefined : json['href'],
        'id': json['id'] == null ? undefined : json['id'],
        'kind': json['kind'] == null ? undefined : json['kind'],
        'label': json['label'] == null ? undefined : json['label'],
        'note': json['note'] == null ? undefined : json['note'],
        'released': json['released'] == null ? undefined : json['released'],
        'tier': json['tier'] == null ? undefined : json['tier'],
        'title': json['title'] == null ? undefined : json['title'],
        'updated': json['updated'] == null ? undefined : json['updated'],
    };
}

export function DocRowToJSON(json: any): DocRow {
    return DocRowToJSONTyped(json, false);
}

export function DocRowToJSONTyped(value?: DocRow | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'attested': value['attested'],
        'href': value['href'],
        'id': value['id'],
        'kind': value['kind'],
        'label': value['label'],
        'note': value['note'],
        'released': value['released'],
        'tier': value['tier'],
        'title': value['title'],
        'updated': value['updated'],
    };
}

