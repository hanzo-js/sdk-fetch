/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface O11yAnnItemView
 */
export interface O11yAnnItemView {
    /**
     * Assignee is the reviewer it is for, omitted when unassigned.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    assignee?: string;
    /**
     * CompletedAt is when it was reviewed, omitted while pending.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    completedAt?: string;
    /**
     * CreatedAt is when it was enqueued, RFC3339 in UTC.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    createdAt?: string;
    /**
     * ID is the item's id.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    id?: string;
    /**
     * ObjectID is the referenced object's id.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    objectId?: string;
    /**
     * ObjectType is what it references: TRACE, OBSERVATION or SESSION.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    objectType?: string;
    /**
     * ObservationID echoes objectId when objectType is OBSERVATION.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    observationId?: string;
    /**
     * QueueID is the queue it belongs to.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    queueId?: string;
    /**
     * SessionID echoes objectId when objectType is SESSION.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    sessionId?: string;
    /**
     * Status is PENDING or COMPLETED.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    status?: string;
    /**
     * TraceID echoes objectId when objectType is TRACE.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    traceId?: string;
    /**
     * UpdatedAt is when it last changed, RFC3339 in UTC.
     * @type {string}
     * @memberof O11yAnnItemView
     */
    updatedAt?: string;
}

/**
 * Check if a given object implements the O11yAnnItemView interface.
 */
export function instanceOfO11yAnnItemView(value: object): value is O11yAnnItemView {
    return true;
}

export function O11yAnnItemViewFromJSON(json: any): O11yAnnItemView {
    return O11yAnnItemViewFromJSONTyped(json, false);
}

export function O11yAnnItemViewFromJSONTyped(json: any, ignoreDiscriminator: boolean): O11yAnnItemView {
    if (json == null) {
        return json;
    }
    return {
        
        'assignee': json['assignee'] == null ? undefined : json['assignee'],
        'completedAt': json['completedAt'] == null ? undefined : json['completedAt'],
        'createdAt': json['createdAt'] == null ? undefined : json['createdAt'],
        'id': json['id'] == null ? undefined : json['id'],
        'objectId': json['objectId'] == null ? undefined : json['objectId'],
        'objectType': json['objectType'] == null ? undefined : json['objectType'],
        'observationId': json['observationId'] == null ? undefined : json['observationId'],
        'queueId': json['queueId'] == null ? undefined : json['queueId'],
        'sessionId': json['sessionId'] == null ? undefined : json['sessionId'],
        'status': json['status'] == null ? undefined : json['status'],
        'traceId': json['traceId'] == null ? undefined : json['traceId'],
        'updatedAt': json['updatedAt'] == null ? undefined : json['updatedAt'],
    };
}

export function O11yAnnItemViewToJSON(json: any): O11yAnnItemView {
    return O11yAnnItemViewToJSONTyped(json, false);
}

export function O11yAnnItemViewToJSONTyped(value?: O11yAnnItemView | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'assignee': value['assignee'],
        'completedAt': value['completedAt'],
        'createdAt': value['createdAt'],
        'id': value['id'],
        'objectId': value['objectId'],
        'objectType': value['objectType'],
        'observationId': value['observationId'],
        'queueId': value['queueId'],
        'sessionId': value['sessionId'],
        'status': value['status'],
        'traceId': value['traceId'],
        'updatedAt': value['updatedAt'],
    };
}

