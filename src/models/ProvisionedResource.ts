/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface ProvisionedResource
 */
export interface ProvisionedResource {
    /**
     * Database is the logical database, collection, index or bucket this
     * resource resolves to on its backend.
     * @type {string}
     * @memberof ProvisionedResource
     */
    database?: string;
    /**
     * Host is the address that actually routes to this resource — a dedicated
     * instance's own in-cluster Service, or the public gateway for a shared one.
     * @type {string}
     * @memberof ProvisionedResource
     */
    host?: string;
    /**
     * ID is the resource's server-minted handle, "rs_"-prefixed.
     * @type {string}
     * @memberof ProvisionedResource
     */
    id?: string;
    /**
     * Kind is the product: sql, vector, datastore, kv, search, s3 or docdb.
     * @type {string}
     * @memberof ProvisionedResource
     */
    kind?: string;
    /**
     * Name is the org-unique slug the caller provisioned the resource under.
     * @type {string}
     * @memberof ProvisionedResource
     */
    name?: string;
    /**
     * Port is the port a client connects to on Host.
     * @type {number}
     * @memberof ProvisionedResource
     */
    port?: number;
    /**
     * Status is "ready", or "provisioning" while a dedicated instance is still
     * being materialized. A dedicated resource's status is reconciled from the
     * operator's live CR before this is answered, so it is never a stale ready.
     * @type {string}
     * @memberof ProvisionedResource
     */
    status?: string;
    /**
     * Username is the credential's user, for the kinds that mint one per
     * resource. Absent for a kind whose backend authenticates with a shared,
     * out-of-band key.
     * @type {string}
     * @memberof ProvisionedResource
     */
    username?: string;
}

/**
 * Check if a given object implements the ProvisionedResource interface.
 */
export function instanceOfProvisionedResource(value: object): value is ProvisionedResource {
    return true;
}

export function ProvisionedResourceFromJSON(json: any): ProvisionedResource {
    return ProvisionedResourceFromJSONTyped(json, false);
}

export function ProvisionedResourceFromJSONTyped(json: any, ignoreDiscriminator: boolean): ProvisionedResource {
    if (json == null) {
        return json;
    }
    return {
        
        'database': json['database'] == null ? undefined : json['database'],
        'host': json['host'] == null ? undefined : json['host'],
        'id': json['id'] == null ? undefined : json['id'],
        'kind': json['kind'] == null ? undefined : json['kind'],
        'name': json['name'] == null ? undefined : json['name'],
        'port': json['port'] == null ? undefined : json['port'],
        'status': json['status'] == null ? undefined : json['status'],
        'username': json['username'] == null ? undefined : json['username'],
    };
}

export function ProvisionedResourceToJSON(json: any): ProvisionedResource {
    return ProvisionedResourceToJSONTyped(json, false);
}

export function ProvisionedResourceToJSONTyped(value?: ProvisionedResource | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'database': value['database'],
        'host': value['host'],
        'id': value['id'],
        'kind': value['kind'],
        'name': value['name'],
        'port': value['port'],
        'status': value['status'],
        'username': value['username'],
    };
}

