/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface SocialPostBody
 */
export interface SocialPostBody {
    /**
     * Channel is the network to publish to: x, facebook, instagram, linkedin,
     * tiktok, youtube or threads. Omitted means x.
     * 
     * Example: "x"
     * @type {string}
     * @memberof SocialPostBody
     */
    channel?: string;
    /**
     * Content is the post's text. Required — an empty body is a 400 — and bounded
     * at 8192 characters, comfortably above every network's own limit.
     * 
     * Example: "Shipping today."
     * @type {string}
     * @memberof SocialPostBody
     */
    content?: string;
    /**
     * Media is the post's attached media as URLs, at most 10, each bounded at 1024
     * characters. Blank entries are dropped. Omitting it CLEARS any stored media.
     * @type {Array<string>}
     * @memberof SocialPostBody
     */
    media?: Array<string>;
    /**
     * ScheduleAt is when to publish, as a unix timestamp in SECONDS. 0 means
     * unscheduled. A negative value is clamped to 0. It only matters for a post
     * whose status is scheduled.
     * @type {number}
     * @memberof SocialPostBody
     */
    scheduleAt?: number;
    /**
     * Status is the post's lifecycle state: draft, scheduled, published or failed.
     * Omitted means draft. The transient publishing claim is never settable here —
     * accepting it from a request would let a caller wedge or replay the guard that
     * stops two publishers double-posting the same row.
     * @type {string}
     * @memberof SocialPostBody
     */
    status?: string;
}

/**
 * Check if a given object implements the SocialPostBody interface.
 */
export function instanceOfSocialPostBody(value: object): value is SocialPostBody {
    return true;
}

export function SocialPostBodyFromJSON(json: any): SocialPostBody {
    return SocialPostBodyFromJSONTyped(json, false);
}

export function SocialPostBodyFromJSONTyped(json: any, ignoreDiscriminator: boolean): SocialPostBody {
    if (json == null) {
        return json;
    }
    return {
        
        'channel': json['channel'] == null ? undefined : json['channel'],
        'content': json['content'] == null ? undefined : json['content'],
        'media': json['media'] == null ? undefined : json['media'],
        'scheduleAt': json['scheduleAt'] == null ? undefined : json['scheduleAt'],
        'status': json['status'] == null ? undefined : json['status'],
    };
}

export function SocialPostBodyToJSON(json: any): SocialPostBody {
    return SocialPostBodyToJSONTyped(json, false);
}

export function SocialPostBodyToJSONTyped(value?: SocialPostBody | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'channel': value['channel'],
        'content': value['content'],
        'media': value['media'],
        'scheduleAt': value['scheduleAt'],
        'status': value['status'],
    };
}

