/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface DriveIn
 */
export interface DriveIn {
    /**
     * Name is the drive's name, matching the same shape a space name does. It
     * becomes the FIRST SEGMENT of every key the drive holds, which is why it may
     * carry no "/".
     * @type {string}
     * @memberof DriveIn
     */
    name?: string;
    /**
     * Space is the space to create the drive in, from the path. It carries NO
     * `url:"-"`, unlike the field below it, and the difference is the whole reason
     * both tags are written out: zip's binder skips a field tagged "-" for EVERY
     * URL source, path params included, so a path-borne value that carried it
     * would arrive empty and the create would refuse a perfectly good address.
     * @type {string}
     * @memberof DriveIn
     */
    space?: string;
}

/**
 * Check if a given object implements the DriveIn interface.
 */
export function instanceOfDriveIn(value: object): value is DriveIn {
    return true;
}

export function DriveInFromJSON(json: any): DriveIn {
    return DriveInFromJSONTyped(json, false);
}

export function DriveInFromJSONTyped(json: any, ignoreDiscriminator: boolean): DriveIn {
    if (json == null) {
        return json;
    }
    return {
        
        'name': json['name'] == null ? undefined : json['name'],
        'space': json['space'] == null ? undefined : json['space'],
    };
}

export function DriveInToJSON(json: any): DriveIn {
    return DriveInToJSONTyped(json, false);
}

export function DriveInToJSONTyped(value?: DriveIn | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'name': value['name'],
        'space': value['space'],
    };
}

