/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface IndexResult
 */
export interface IndexResult {
    /**
     * Chunks is how many AST-boundary chunks the repo holds after this pass.
     * @type {number}
     * @memberof IndexResult
     */
    chunks?: number;
    /**
     * Files is how many files the repo holds after this pass.
     * @type {number}
     * @memberof IndexResult
     */
    files?: number;
    /**
     * Indexed is how many files were parsed and written on this pass.
     * @type {number}
     * @memberof IndexResult
     */
    indexed?: number;
    /**
     * Pruned is how many stored files were deleted because prune was set and they
     * were absent from the request.
     * @type {number}
     * @memberof IndexResult
     */
    pruned?: number;
    /**
     * Repo is the repository that was indexed.
     * @type {string}
     * @memberof IndexResult
     */
    repo?: string;
    /**
     * Semantic reports whether the semantic tier was available for this pass. When
     * false the index is lexical + symbolic only and hybrid search still works.
     * @type {boolean}
     * @memberof IndexResult
     */
    semantic?: boolean;
    /**
     * Skipped is how many files were unchanged by content hash and left alone.
     * @type {number}
     * @memberof IndexResult
     */
    skipped?: number;
    /**
     * Symbols is how many symbol definitions the repo holds after this pass.
     * @type {number}
     * @memberof IndexResult
     */
    symbols?: number;
    /**
     * Vectors is how many of those chunks carry an embedding.
     * @type {number}
     * @memberof IndexResult
     */
    vectors?: number;
}

/**
 * Check if a given object implements the IndexResult interface.
 */
export function instanceOfIndexResult(value: object): value is IndexResult {
    return true;
}

export function IndexResultFromJSON(json: any): IndexResult {
    return IndexResultFromJSONTyped(json, false);
}

export function IndexResultFromJSONTyped(json: any, ignoreDiscriminator: boolean): IndexResult {
    if (json == null) {
        return json;
    }
    return {
        
        'chunks': json['chunks'] == null ? undefined : json['chunks'],
        'files': json['files'] == null ? undefined : json['files'],
        'indexed': json['indexed'] == null ? undefined : json['indexed'],
        'pruned': json['pruned'] == null ? undefined : json['pruned'],
        'repo': json['repo'] == null ? undefined : json['repo'],
        'semantic': json['semantic'] == null ? undefined : json['semantic'],
        'skipped': json['skipped'] == null ? undefined : json['skipped'],
        'symbols': json['symbols'] == null ? undefined : json['symbols'],
        'vectors': json['vectors'] == null ? undefined : json['vectors'],
    };
}

export function IndexResultToJSON(json: any): IndexResult {
    return IndexResultToJSONTyped(json, false);
}

export function IndexResultToJSONTyped(value?: IndexResult | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'chunks': value['chunks'],
        'files': value['files'],
        'indexed': value['indexed'],
        'pruned': value['pruned'],
        'repo': value['repo'],
        'semantic': value['semantic'],
        'skipped': value['skipped'],
        'symbols': value['symbols'],
        'vectors': value['vectors'],
    };
}

