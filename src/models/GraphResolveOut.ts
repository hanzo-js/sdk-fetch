/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
import type { WireFact } from './WireFact.js';
import {
    WireFactFromJSON,
    WireFactFromJSONTyped,
    WireFactToJSON,
    WireFactToJSONTyped,
} from './WireFact.js';

/**
 * 
 * @export
 * @interface GraphResolveOut
 */
export interface GraphResolveOut {
    /**
     * AsOf is the instant this answer was taken at, RFC 3339: the one asked for,
     * or the server's clock when none was.
     * @type {string}
     * @memberof GraphResolveOut
     */
    asOf?: string;
    /**
     * Conflicts is every OTHER assertion knowable at AsOf, strongest first. They
     * are not all disagreements: one that repeats the winner's value ranks below
     * it and is listed here too.
     * @type {Array<WireFact>}
     * @memberof GraphResolveOut
     */
    conflicts?: Array<WireFact>;
    /**
     * Contested is true when at least one conflict claims a value different from
     * the winner's. Any number of conflicts that all agree leaves it false.
     * @type {boolean}
     * @memberof GraphResolveOut
     */
    contested?: boolean;
    /**
     * Entity is the entity the question named, echoed so a stored answer still
     * says what it is about.
     * @type {string}
     * @memberof GraphResolveOut
     */
    entity?: string;
    /**
     * Known is false when this plane held nothing knowable at AsOf. That is an
     * answer, not an error.
     * @type {boolean}
     * @memberof GraphResolveOut
     */
    known?: boolean;
    /**
     * Relation is the relation the question named, echoed for the same reason.
     * @type {string}
     * @memberof GraphResolveOut
     */
    relation?: string;
    /**
     * Truncated says this pair holds more assertions than one read returns, so
     * the winner was decided from the most recent ceiling-full of them. It is
     * reported because a provenance plane that trims silently is a plane that
     * answers confidently and wrongly; narrow the question with as_of to see
     * what it dropped.
     * @type {boolean}
     * @memberof GraphResolveOut
     */
    truncated?: boolean;
    /**
     * Winner is the assertion in force — the strongest of those knowable at AsOf
     * under the order `rule` names. Absent exactly when Known is false.
     * @type {WireFact}
     * @memberof GraphResolveOut
     */
    winner?: WireFact;
}

/**
 * Check if a given object implements the GraphResolveOut interface.
 */
export function instanceOfGraphResolveOut(value: object): value is GraphResolveOut {
    return true;
}

export function GraphResolveOutFromJSON(json: any): GraphResolveOut {
    return GraphResolveOutFromJSONTyped(json, false);
}

export function GraphResolveOutFromJSONTyped(json: any, ignoreDiscriminator: boolean): GraphResolveOut {
    if (json == null) {
        return json;
    }
    return {
        
        'asOf': json['as_of'] == null ? undefined : json['as_of'],
        'conflicts': json['conflicts'] == null ? undefined : ((json['conflicts'] as Array<any>).map(WireFactFromJSON)),
        'contested': json['contested'] == null ? undefined : json['contested'],
        'entity': json['entity'] == null ? undefined : json['entity'],
        'known': json['known'] == null ? undefined : json['known'],
        'relation': json['relation'] == null ? undefined : json['relation'],
        'truncated': json['truncated'] == null ? undefined : json['truncated'],
        'winner': json['winner'] == null ? undefined : WireFactFromJSON(json['winner']),
    };
}

export function GraphResolveOutToJSON(json: any): GraphResolveOut {
    return GraphResolveOutToJSONTyped(json, false);
}

export function GraphResolveOutToJSONTyped(value?: GraphResolveOut | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'as_of': value['asOf'],
        'conflicts': value['conflicts'] == null ? undefined : ((value['conflicts'] as Array<any>).map(WireFactToJSON)),
        'contested': value['contested'],
        'entity': value['entity'],
        'known': value['known'],
        'relation': value['relation'],
        'truncated': value['truncated'],
        'winner': WireFactToJSON(value['winner']),
    };
}

