/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface RiskLineage
 */
export interface RiskLineage {
    /**
     * 
     * @type {string}
     * @memberof RiskLineage
     */
    dataset?: string;
    /**
     * Digest is the version's fingerprint, repeated here so a lineage answer is
     * self-contained.
     * @type {string}
     * @memberof RiskLineage
     */
    digest?: string;
    /**
     * From and To are the window actually read — To is the window's end pulled
     * back by the maturity horizon, which is usually earlier than the spec's.
     * @type {string}
     * @memberof RiskLineage
     */
    from?: string;
    /**
     * Holds is what the source holds for the same window NOW. The difference
     * between it and Rows is the whole of the reproducibility claim.
     * @type {number}
     * @memberof RiskLineage
     */
    holds?: number;
    /**
     * Oversize is how many subjects the window held that were too large to
     * represent when this version was built. It is part of the fingerprint, so it
     * is part of what "reproducible" is measured over.
     * @type {number}
     * @memberof RiskLineage
     */
    oversize?: number;
    /**
     * 
     * @type {string}
     * @memberof RiskLineage
     */
    refusal?: string;
    /**
     * Reproducible is true when the source still holds what this version was built
     * from. Refusal says why not, when it is false.
     * @type {boolean}
     * @memberof RiskLineage
     */
    reproducible?: boolean;
    /**
     * Retention is the source's own expiry rule as the store reports it, read at
     * materialisation time rather than assumed. A source whose retention is
     * shorter than this window cannot re-derive it.
     * @type {string}
     * @memberof RiskLineage
     */
    retention?: string;
    /**
     * Rows and Subjects are what the source held for that window at
     * materialisation time.
     * @type {number}
     * @memberof RiskLineage
     */
    rows?: number;
    /**
     * Share is the fraction of subjects admitted, in thousandths.
     * @type {number}
     * @memberof RiskLineage
     */
    share?: number;
    /**
     * Source is the plane the rows were derived from.
     * @type {string}
     * @memberof RiskLineage
     */
    source?: string;
    /**
     * 
     * @type {number}
     * @memberof RiskLineage
     */
    subjects?: number;
    /**
     * 
     * @type {string}
     * @memberof RiskLineage
     */
    to?: string;
    /**
     * 
     * @type {number}
     * @memberof RiskLineage
     */
    version?: number;
}

/**
 * Check if a given object implements the RiskLineage interface.
 */
export function instanceOfRiskLineage(value: object): value is RiskLineage {
    return true;
}

export function RiskLineageFromJSON(json: any): RiskLineage {
    return RiskLineageFromJSONTyped(json, false);
}

export function RiskLineageFromJSONTyped(json: any, ignoreDiscriminator: boolean): RiskLineage {
    if (json == null) {
        return json;
    }
    return {
        
        'dataset': json['dataset'] == null ? undefined : json['dataset'],
        'digest': json['digest'] == null ? undefined : json['digest'],
        'from': json['from'] == null ? undefined : json['from'],
        'holds': json['holds'] == null ? undefined : json['holds'],
        'oversize': json['oversize'] == null ? undefined : json['oversize'],
        'refusal': json['refusal'] == null ? undefined : json['refusal'],
        'reproducible': json['reproducible'] == null ? undefined : json['reproducible'],
        'retention': json['retention'] == null ? undefined : json['retention'],
        'rows': json['rows'] == null ? undefined : json['rows'],
        'share': json['share'] == null ? undefined : json['share'],
        'source': json['source'] == null ? undefined : json['source'],
        'subjects': json['subjects'] == null ? undefined : json['subjects'],
        'to': json['to'] == null ? undefined : json['to'],
        'version': json['version'] == null ? undefined : json['version'],
    };
}

export function RiskLineageToJSON(json: any): RiskLineage {
    return RiskLineageToJSONTyped(json, false);
}

export function RiskLineageToJSONTyped(value?: RiskLineage | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'dataset': value['dataset'],
        'digest': value['digest'],
        'from': value['from'],
        'holds': value['holds'],
        'oversize': value['oversize'],
        'refusal': value['refusal'],
        'reproducible': value['reproducible'],
        'retention': value['retention'],
        'rows': value['rows'],
        'share': value['share'],
        'source': value['source'],
        'subjects': value['subjects'],
        'to': value['to'],
        'version': value['version'],
    };
}

