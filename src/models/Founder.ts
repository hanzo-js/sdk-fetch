/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface Founder
 */
export interface Founder {
    /**
     * DecidedBy is who settled a terminal KYC status: the provider name, or a
     * reviewer's user id.
     * @type {string}
     * @memberof Founder
     */
    decidedBy?: string;
    /**
     * Email is the founder's email, and the key a KYC decision addresses a founder
     * by — POST /v1/company/kyc/decision matches on it.
     * @type {string}
     * @memberof Founder
     */
    email?: string;
    /**
     * EquityBps is the founder's ownership in basis points, 0–10000 (1% == 100 bps,
     * so 10000 is the whole company). The founders' shares seed the cap-table genesis.
     * @type {number}
     * @memberof Founder
     */
    equityBps?: number;
    /**
     * KYCRef is the idv provider's session reference for this founder.
     * @type {string}
     * @memberof Founder
     */
    kycRef?: string;
    /**
     * KYCStatus is the founder's identity-verification state: pending, verified (a
     * real idv provider reported a pass), reviewer_confirmed (a privileged reviewer
     * confirmed out-of-band) or failed. The payment stage is unreachable until every
     * founder passes.
     * @type {string}
     * @memberof Founder
     */
    kycStatus?: string;
    /**
     * Name is the founder's full legal name, as it appears on the formation documents.
     * @type {string}
     * @memberof Founder
     */
    name?: string;
}

/**
 * Check if a given object implements the Founder interface.
 */
export function instanceOfFounder(value: object): value is Founder {
    return true;
}

export function FounderFromJSON(json: any): Founder {
    return FounderFromJSONTyped(json, false);
}

export function FounderFromJSONTyped(json: any, ignoreDiscriminator: boolean): Founder {
    if (json == null) {
        return json;
    }
    return {
        
        'decidedBy': json['decidedBy'] == null ? undefined : json['decidedBy'],
        'email': json['email'] == null ? undefined : json['email'],
        'equityBps': json['equityBps'] == null ? undefined : json['equityBps'],
        'kycRef': json['kycRef'] == null ? undefined : json['kycRef'],
        'kycStatus': json['kycStatus'] == null ? undefined : json['kycStatus'],
        'name': json['name'] == null ? undefined : json['name'],
    };
}

export function FounderToJSON(json: any): Founder {
    return FounderToJSONTyped(json, false);
}

export function FounderToJSONTyped(value?: Founder | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'decidedBy': value['decidedBy'],
        'email': value['email'],
        'equityBps': value['equityBps'],
        'kycRef': value['kycRef'],
        'kycStatus': value['kycStatus'],
        'name': value['name'],
    };
}

