/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface RiskLabelVocabulary
 */
export interface RiskLabelVocabulary {
    /**
     * Dispositions is the closed set a write's `disposition` must be drawn from,
     * published in full so a caller can validate a batch before filing it instead of
     * discovering a refusal per member: "productive", "unproductive", and "" — the
     * EMPTY STRING is a member and means an explicit unjudged, so a client that
     * filters empties out of this list drops a third of the vocabulary and can never
     * file "we looked and could not say". They are the AML engine's own spelling,
     * verbatim, which is what lets a replay there report against these values.
     * @type {Array<string>}
     * @memberof RiskLabelVocabulary
     */
    dispositions?: Array<string>;
    /**
     * Kinds, Dispositions and Sources are the closed vocabularies. A value
     * outside them is refused at the endpoint.
     * @type {Array<string>}
     * @memberof RiskLabelVocabulary
     */
    kinds?: Array<string>;
    /**
     * Precedence is the sources in the order that resolves a conflict, strongest
     * first. It is DERIVED from the same declaration the resolver reads, so the
     * published order is the enforced order and cannot drift from it.
     * @type {Array<string>}
     * @memberof RiskLabelVocabulary
     */
    precedence?: Array<string>;
    /**
     * Retention is the platform floor in days: no tenant may dispose of a label
     * younger than this, because a label can be the input to an adverse action.
     * @type {number}
     * @memberof RiskLabelVocabulary
     */
    retention?: number;
    /**
     * Rule states the tie-breaks below rank, in order, so a caller reading a
     * contested resolution can reproduce it.
     * @type {Array<string>}
     * @memberof RiskLabelVocabulary
     */
    rule?: Array<string>;
}

/**
 * Check if a given object implements the RiskLabelVocabulary interface.
 */
export function instanceOfRiskLabelVocabulary(value: object): value is RiskLabelVocabulary {
    return true;
}

export function RiskLabelVocabularyFromJSON(json: any): RiskLabelVocabulary {
    return RiskLabelVocabularyFromJSONTyped(json, false);
}

export function RiskLabelVocabularyFromJSONTyped(json: any, ignoreDiscriminator: boolean): RiskLabelVocabulary {
    if (json == null) {
        return json;
    }
    return {
        
        'dispositions': json['dispositions'] == null ? undefined : json['dispositions'],
        'kinds': json['kinds'] == null ? undefined : json['kinds'],
        'precedence': json['precedence'] == null ? undefined : json['precedence'],
        'retention': json['retention'] == null ? undefined : json['retention'],
        'rule': json['rule'] == null ? undefined : json['rule'],
    };
}

export function RiskLabelVocabularyToJSON(json: any): RiskLabelVocabulary {
    return RiskLabelVocabularyToJSONTyped(json, false);
}

export function RiskLabelVocabularyToJSONTyped(value?: RiskLabelVocabulary | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'dispositions': value['dispositions'],
        'kinds': value['kinds'],
        'precedence': value['precedence'],
        'retention': value['retention'],
        'rule': value['rule'],
    };
}

