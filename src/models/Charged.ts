/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface Charged
 */
export interface Charged {
    /**
     * BalanceCents is the subject's balance AFTER the charge settled, in cents, so
     * a caller does not have to re-read to show the new number.
     * @type {number}
     * @memberof Charged
     */
    balanceCents?: number;
    /**
     * ProcessorRef is the payment processor's own reference. It is the only field
     * that proves money moved at the GATEWAY rather than merely in our ledger,
     * which is why it is answered and not only logged. Absent where the processor
     * returned none.
     * @type {string}
     * @memberof Charged
     */
    processorRef?: string;
    /**
     * Status is how the charge ended. Read it rather than inferring success from
     * the HTTP status: the call succeeded whenever this field is present, and what
     * the PROCESSOR did is what this says.
     * @type {string}
     * @memberof Charged
     */
    status?: string;
    /**
     * Test states which bucket was credited — sandbox money or real money — so no
     * reader has to guess whether a receipt is real. Sandbox and live funds are
     * physically separate ledgers, and a reader that conflates them restates the
     * company's revenue.
     * @type {boolean}
     * @memberof Charged
     */
    test?: boolean;
    /**
     * TransactionID is the ledger entry this charge created. It is the handle a
     * later read or a refund names, and it is minted by the ledger rather than by
     * the caller.
     * @type {string}
     * @memberof Charged
     */
    transactionId?: string;
}

/**
 * Check if a given object implements the Charged interface.
 */
export function instanceOfCharged(value: object): value is Charged {
    return true;
}

export function ChargedFromJSON(json: any): Charged {
    return ChargedFromJSONTyped(json, false);
}

export function ChargedFromJSONTyped(json: any, ignoreDiscriminator: boolean): Charged {
    if (json == null) {
        return json;
    }
    return {
        
        'balanceCents': json['balanceCents'] == null ? undefined : json['balanceCents'],
        'processorRef': json['processorRef'] == null ? undefined : json['processorRef'],
        'status': json['status'] == null ? undefined : json['status'],
        'test': json['test'] == null ? undefined : json['test'],
        'transactionId': json['transactionId'] == null ? undefined : json['transactionId'],
    };
}

export function ChargedToJSON(json: any): Charged {
    return ChargedToJSONTyped(json, false);
}

export function ChargedToJSONTyped(value?: Charged | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'balanceCents': value['balanceCents'],
        'processorRef': value['processorRef'],
        'status': value['status'],
        'test': value['test'],
        'transactionId': value['transactionId'],
    };
}

