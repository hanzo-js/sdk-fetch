/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface OpenReq
 */
export interface OpenReq {
    /**
     * Base is the branch the work is proposed INTO, by short name. Defaults to
     * the repo's default branch, which is where a proposal goes when nobody says
     * otherwise.
     * @type {string}
     * @memberof OpenReq
     */
    base?: string;
    /**
     * Body is the longer description. Optional.
     * @type {string}
     * @memberof OpenReq
     */
    body?: string;
    /**
     * Head is the branch holding the work, by short name (agent/fix-503).
     * Required, and must already exist.
     * @type {string}
     * @memberof OpenReq
     */
    head?: string;
    /**
     * Name is the repo the proposal belongs to, from the :name path segment.
     * @type {string}
     * @memberof OpenReq
     */
    name?: string;
    /**
     * Title is the one-line summary of what is being proposed. Required.
     * @type {string}
     * @memberof OpenReq
     */
    title?: string;
}

/**
 * Check if a given object implements the OpenReq interface.
 */
export function instanceOfOpenReq(value: object): value is OpenReq {
    return true;
}

export function OpenReqFromJSON(json: any): OpenReq {
    return OpenReqFromJSONTyped(json, false);
}

export function OpenReqFromJSONTyped(json: any, ignoreDiscriminator: boolean): OpenReq {
    if (json == null) {
        return json;
    }
    return {
        
        'base': json['base'] == null ? undefined : json['base'],
        'body': json['body'] == null ? undefined : json['body'],
        'head': json['head'] == null ? undefined : json['head'],
        'name': json['name'] == null ? undefined : json['name'],
        'title': json['title'] == null ? undefined : json['title'],
    };
}

export function OpenReqToJSON(json: any): OpenReq {
    return OpenReqToJSONTyped(json, false);
}

export function OpenReqToJSONTyped(value?: OpenReq | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'base': value['base'],
        'body': value['body'],
        'head': value['head'],
        'name': value['name'],
        'title': value['title'],
    };
}

