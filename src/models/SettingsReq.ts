/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface SettingsReq
 */
export interface SettingsReq {
    /**
     * Config is the product's non-secret configuration, stored verbatim. Bounded at
     * 64 KiB once serialized. Omit it to store an empty object.
     * @type {{ [key: string]: object; }}
     * @memberof SettingsReq
     */
    config?: { [key: string]: object; };
    /**
     * Product is the catalog slug, from the PATH. zip binds the path last, so the
     * URL names the product being written whatever a body field claims.
     * @type {string}
     * @memberof SettingsReq
     */
    product?: string;
    /**
     * Secrets are the secret fields, by name. Each VALUE is sealed into KMS and
     * never reaches this deployment's database; a value that is empty or equal to
     * the mask the read path returns means "unchanged" and is skipped, so a console
     * round-trip cannot blank a stored secret. A key must match
     * ^[a-z0-9][a-z0-9._-]{0,62}$, a value is bounded at 8 KiB, and an org may hold
     * at most 64 secret fields per product.
     * @type {{ [key: string]: string; }}
     * @memberof SettingsReq
     */
    secrets?: { [key: string]: string; };
}

/**
 * Check if a given object implements the SettingsReq interface.
 */
export function instanceOfSettingsReq(value: object): value is SettingsReq {
    return true;
}

export function SettingsReqFromJSON(json: any): SettingsReq {
    return SettingsReqFromJSONTyped(json, false);
}

export function SettingsReqFromJSONTyped(json: any, ignoreDiscriminator: boolean): SettingsReq {
    if (json == null) {
        return json;
    }
    return {
        
        'config': json['config'] == null ? undefined : json['config'],
        'product': json['product'] == null ? undefined : json['product'],
        'secrets': json['secrets'] == null ? undefined : json['secrets'],
    };
}

export function SettingsReqToJSON(json: any): SettingsReq {
    return SettingsReqToJSONTyped(json, false);
}

export function SettingsReqToJSONTyped(value?: SettingsReq | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'config': value['config'],
        'product': value['product'],
        'secrets': value['secrets'],
    };
}

