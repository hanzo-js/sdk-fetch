/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay routes, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface SeoBacklinkOut
 */
export interface SeoBacklinkOut {
    /**
     * Backlinks is how many links point at it.
     * @type {number}
     * @memberof SeoBacklinkOut
     */
    backlinks?: number;
    /**
     * Broken is how many of those links point at something that no longer answers.
     * @type {number}
     * @memberof SeoBacklinkOut
     */
    broken?: number;
    /**
     * Cost is what this call cost, in USD, as an exact decimal string.
     * @type {string}
     * @memberof SeoBacklinkOut
     */
    cost?: string;
    /**
     * Domains is how many distinct sites those links come from — the number that
     * matters, since a thousand links from one site is one site.
     * @type {number}
     * @memberof SeoBacklinkOut
     */
    domains?: number;
    /**
     * FirstSeen is when the upstream first saw a link to this target, RFC 3339.
     * @type {string}
     * @memberof SeoBacklinkOut
     */
    firstSeen?: string;
    /**
     * Pages is how many distinct pages link in.
     * @type {number}
     * @memberof SeoBacklinkOut
     */
    pages?: number;
    /**
     * Rank is the upstream's authority score for the target, 0 to 1000.
     * @type {number}
     * @memberof SeoBacklinkOut
     */
    rank?: number;
    /**
     * Spam is the share of the profile judged spam, 0 to 100.
     * @type {number}
     * @memberof SeoBacklinkOut
     */
    spam?: number;
    /**
     * Target is the target as the upstream resolved it.
     * @type {string}
     * @memberof SeoBacklinkOut
     */
    target?: string;
}

/**
 * Check if a given object implements the SeoBacklinkOut interface.
 */
export function instanceOfSeoBacklinkOut(value: object): value is SeoBacklinkOut {
    return true;
}

export function SeoBacklinkOutFromJSON(json: any): SeoBacklinkOut {
    return SeoBacklinkOutFromJSONTyped(json, false);
}

export function SeoBacklinkOutFromJSONTyped(json: any, ignoreDiscriminator: boolean): SeoBacklinkOut {
    if (json == null) {
        return json;
    }
    return {
        
        'backlinks': json['backlinks'] == null ? undefined : json['backlinks'],
        'broken': json['broken'] == null ? undefined : json['broken'],
        'cost': json['cost'] == null ? undefined : json['cost'],
        'domains': json['domains'] == null ? undefined : json['domains'],
        'firstSeen': json['firstSeen'] == null ? undefined : json['firstSeen'],
        'pages': json['pages'] == null ? undefined : json['pages'],
        'rank': json['rank'] == null ? undefined : json['rank'],
        'spam': json['spam'] == null ? undefined : json['spam'],
        'target': json['target'] == null ? undefined : json['target'],
    };
}

export function SeoBacklinkOutToJSON(json: any): SeoBacklinkOut {
    return SeoBacklinkOutToJSONTyped(json, false);
}

export function SeoBacklinkOutToJSONTyped(value?: SeoBacklinkOut | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'backlinks': value['backlinks'],
        'broken': value['broken'],
        'cost': value['cost'],
        'domains': value['domains'],
        'firstSeen': value['firstSeen'],
        'pages': value['pages'],
        'rank': value['rank'],
        'spam': value['spam'],
        'target': value['target'],
    };
}

