/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * The Hanzo Cloud API as a customer calls it: every operation under /v1/ except the operator\'s admin product, relay doors, legacy spellings and capabilities still reached by flag. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
import type { GraphNode } from './GraphNode.js';
import {
    GraphNodeFromJSON,
    GraphNodeFromJSONTyped,
    GraphNodeToJSON,
    GraphNodeToJSONTyped,
} from './GraphNode.js';
import type { GraphEdge } from './GraphEdge.js';
import {
    GraphEdgeFromJSON,
    GraphEdgeFromJSONTyped,
    GraphEdgeToJSON,
    GraphEdgeToJSONTyped,
} from './GraphEdge.js';

/**
 * 
 * @export
 * @interface GraphOut
 */
export interface GraphOut {
    /**
     * Degraded is true when the store was unreachable and this graph is honestly
     * empty rather than wrong. Absent on a normal answer.
     * @type {boolean}
     * @memberof GraphOut
     */
    degraded?: boolean;
    /**
     * Edges are the parent tree, the resolved wikilinks and the connector provenance.
     * @type {Array<GraphEdge>}
     * @memberof GraphOut
     */
    edges?: Array<GraphEdge>;
    /**
     * Nodes are the pages, memories, sources, connectors and unresolved link targets.
     * @type {Array<GraphNode>}
     * @memberof GraphOut
     */
    nodes?: Array<GraphNode>;
}

/**
 * Check if a given object implements the GraphOut interface.
 */
export function instanceOfGraphOut(value: object): value is GraphOut {
    return true;
}

export function GraphOutFromJSON(json: any): GraphOut {
    return GraphOutFromJSONTyped(json, false);
}

export function GraphOutFromJSONTyped(json: any, ignoreDiscriminator: boolean): GraphOut {
    if (json == null) {
        return json;
    }
    return {
        
        'degraded': json['degraded'] == null ? undefined : json['degraded'],
        'edges': json['edges'] == null ? undefined : ((json['edges'] as Array<any>).map(GraphEdgeFromJSON)),
        'nodes': json['nodes'] == null ? undefined : ((json['nodes'] as Array<any>).map(GraphNodeFromJSON)),
    };
}

export function GraphOutToJSON(json: any): GraphOut {
    return GraphOutToJSONTyped(json, false);
}

export function GraphOutToJSONTyped(value?: GraphOut | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'degraded': value['degraded'],
        'edges': value['edges'] == null ? undefined : ((value['edges'] as Array<any>).map(GraphEdgeToJSON)),
        'nodes': value['nodes'] == null ? undefined : ((value['nodes'] as Array<any>).map(GraphNodeToJSON)),
    };
}

