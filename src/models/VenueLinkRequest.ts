/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
/**
 * 
 * @export
 * @interface VenueLinkRequest
 */
export interface VenueLinkRequest {
    /**
     * ClientID is the Azure AD application id. Azure only.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    clientId?: string;
    /**
     * ClientSecret selects the service-principal flow. LEAVING IT OUT selects
     * keyless workload identity federation instead, so omitting it is a choice
     * rather than an omission. Azure only.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    clientSecret?: string;
    /**
     * CredentialJSON is a Google credentials document — an external_account
     * (workload identity federation, keyless) or a service-account key. GCP only.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    credentialJson?: string;
    /**
     * ExternalID pins that role assumption to Hanzo, which is what closes the
     * confused-deputy hole. AWS only.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    externalId?: string;
    /**
     * Label is the org-chosen name for this account within the provider, which is
     * how a second account at the same provider is addressed later. Empty means
     * "default"; anything outside 1–64 of [A-Za-z0-9._-] is refused.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    label?: string;
    /**
     * ProjectIDs bounds the GKE cluster sweep. GCP only.
     * @type {Array<string>}
     * @memberof VenueLinkRequest
     */
    projectIds?: Array<string>;
    /**
     * Provider is the cloud being linked, from the path: digitalocean, aws, gcp
     * or azure.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    provider?: string;
    /**
     * Regions bounds the AWS EKS cluster sweep. AWS only.
     * @type {Array<string>}
     * @memberof VenueLinkRequest
     */
    regions?: Array<string>;
    /**
     * RoleARN is the AWS role Hanzo assumes into the account — the keyless path,
     * so no access key is ever stored.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    roleArn?: string;
    /**
     * SubscriptionIDs bounds the AKS cluster sweep. Azure only.
     * @type {Array<string>}
     * @memberof VenueLinkRequest
     */
    subscriptionIds?: Array<string>;
    /**
     * TenantID is the Azure AD tenant of the app. Azure only.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    tenantId?: string;
    /**
     * Token is the DigitalOcean personal access token. DigitalOcean only, and it
     * is the one provider that requires storing a secret.
     * @type {string}
     * @memberof VenueLinkRequest
     */
    token?: string;
}

/**
 * Check if a given object implements the VenueLinkRequest interface.
 */
export function instanceOfVenueLinkRequest(value: object): value is VenueLinkRequest {
    return true;
}

export function VenueLinkRequestFromJSON(json: any): VenueLinkRequest {
    return VenueLinkRequestFromJSONTyped(json, false);
}

export function VenueLinkRequestFromJSONTyped(json: any, ignoreDiscriminator: boolean): VenueLinkRequest {
    if (json == null) {
        return json;
    }
    return {
        
        'clientId': json['clientId'] == null ? undefined : json['clientId'],
        'clientSecret': json['clientSecret'] == null ? undefined : json['clientSecret'],
        'credentialJson': json['credentialJson'] == null ? undefined : json['credentialJson'],
        'externalId': json['externalId'] == null ? undefined : json['externalId'],
        'label': json['label'] == null ? undefined : json['label'],
        'projectIds': json['projectIds'] == null ? undefined : json['projectIds'],
        'provider': json['provider'] == null ? undefined : json['provider'],
        'regions': json['regions'] == null ? undefined : json['regions'],
        'roleArn': json['roleArn'] == null ? undefined : json['roleArn'],
        'subscriptionIds': json['subscriptionIds'] == null ? undefined : json['subscriptionIds'],
        'tenantId': json['tenantId'] == null ? undefined : json['tenantId'],
        'token': json['token'] == null ? undefined : json['token'],
    };
}

export function VenueLinkRequestToJSON(json: any): VenueLinkRequest {
    return VenueLinkRequestToJSONTyped(json, false);
}

export function VenueLinkRequestToJSONTyped(value?: VenueLinkRequest | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'clientId': value['clientId'],
        'clientSecret': value['clientSecret'],
        'credentialJson': value['credentialJson'],
        'externalId': value['externalId'],
        'label': value['label'],
        'projectIds': value['projectIds'],
        'provider': value['provider'],
        'regions': value['regions'],
        'roleArn': value['roleArn'],
        'subscriptionIds': value['subscriptionIds'],
        'tenantId': value['tenantId'],
        'token': value['token'],
    };
}

