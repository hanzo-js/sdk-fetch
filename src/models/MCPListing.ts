/* tslint:disable */
/* eslint-disable */
/**
 * Hanzo Cloud API
 * Composed from each subsystem\'s own projection of its router, in the fleet\'s mount order — every operation below is a route the subsystem that publishes it registered. Tagged by product: the first path segment after /v1/.
 *
 * The version of the OpenAPI document: v1
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 */

import { mapValues } from '../runtime.js';
import type { MCPRemote } from './MCPRemote.js';
import {
    MCPRemoteFromJSON,
    MCPRemoteFromJSONTyped,
    MCPRemoteToJSON,
    MCPRemoteToJSONTyped,
} from './MCPRemote.js';
import type { MCPPackage } from './MCPPackage.js';
import {
    MCPPackageFromJSON,
    MCPPackageFromJSONTyped,
    MCPPackageToJSON,
    MCPPackageToJSONTyped,
} from './MCPPackage.js';

/**
 * 
 * @export
 * @interface MCPListing
 */
export interface MCPListing {
    /**
     * Description is the publisher's one-line summary.
     * @type {string}
     * @memberof MCPListing
     */
    description?: string;
    /**
     * Featured puts the listing on the front of the shelf. Curation.
     * @type {boolean}
     * @memberof MCPListing
     */
    featured?: boolean;
    /**
     * Hidden keeps the listing out of the org-visible catalog. Curation: a sync
     * never changes it. Only a SuperAdmin sets it, and only a SuperAdmin sees a
     * hidden entry listed.
     * @type {boolean}
     * @memberof MCPListing
     */
    hidden?: boolean;
    /**
     * ID addresses the listing in a URL. It is the reverse-DNS NAME with its one
     * slash written as an underscore — reversible, because a namespace never
     * contains an underscore — so the id is readable and stable rather than a
     * hash that means nothing to whoever reads a link.
     * @type {string}
     * @memberof MCPListing
     */
    id?: string;
    /**
     * Logo is the brand mark to render for the listing — the publisher's icon when
     * the entry carries one, or the one an admin set. Curation.
     * @type {string}
     * @memberof MCPListing
     */
    logo?: string;
    /**
     * Name is the publisher's reverse-DNS name, e.g. "com.stripe/mcp".
     * @type {string}
     * @memberof MCPListing
     */
    name?: string;
    /**
     * Official is whether this is the vendor's OWN server rather than someone
     * else's copy of it. Derived on every sync (see isOfficial) until a
     * SuperAdmin sets it explicitly, after which the admin's answer stands.
     * @type {boolean}
     * @memberof MCPListing
     */
    official?: boolean;
    /**
     * Packages are the runnable package forms — npm, pypi, oci — each with the
     * runtime that launches it and the transport it then speaks.
     * @type {Array<MCPPackage>}
     * @memberof MCPListing
     */
    packages?: Array<MCPPackage>;
    /**
     * Registry is the upstream this row was synced from.
     * @type {string}
     * @memberof MCPListing
     */
    registry?: string;
    /**
     * Remotes are the hosted endpoints the publisher serves the server at.
     * @type {Array<MCPRemote>}
     * @memberof MCPListing
     */
    remotes?: Array<MCPRemote>;
    /**
     * Repo is the source repository URL, when the entry names one.
     * @type {string}
     * @memberof MCPListing
     */
    repo?: string;
    /**
     * Site is the project's homepage, when the entry names one.
     * @type {string}
     * @memberof MCPListing
     */
    site?: string;
    /**
     * Synced is when this row was last confirmed against upstream, Unix seconds.
     * @type {number}
     * @memberof MCPListing
     */
    synced?: number;
    /**
     * Title is the human-readable display name, when the entry carries one.
     * @type {string}
     * @memberof MCPListing
     */
    title?: string;
    /**
     * Transports are the distinct transports this server can be reached over,
     * sorted: some of "stdio", "streamable-http", "sse". A listing with
     * "streamable-http" is one an org can enable here and now; a listing that is
     * only "stdio" needs a process to run it.
     * @type {Array<string>}
     * @memberof MCPListing
     */
    transports?: Array<string>;
    /**
     * Vendor is the namespace half of Name — the publisher, e.g. "com.stripe".
     * @type {string}
     * @memberof MCPListing
     */
    vendor?: string;
    /**
     * Version is the published version of this listing.
     * @type {string}
     * @memberof MCPListing
     */
    version?: string;
}

/**
 * Check if a given object implements the MCPListing interface.
 */
export function instanceOfMCPListing(value: object): value is MCPListing {
    return true;
}

export function MCPListingFromJSON(json: any): MCPListing {
    return MCPListingFromJSONTyped(json, false);
}

export function MCPListingFromJSONTyped(json: any, ignoreDiscriminator: boolean): MCPListing {
    if (json == null) {
        return json;
    }
    return {
        
        'description': json['description'] == null ? undefined : json['description'],
        'featured': json['featured'] == null ? undefined : json['featured'],
        'hidden': json['hidden'] == null ? undefined : json['hidden'],
        'id': json['id'] == null ? undefined : json['id'],
        'logo': json['logo'] == null ? undefined : json['logo'],
        'name': json['name'] == null ? undefined : json['name'],
        'official': json['official'] == null ? undefined : json['official'],
        'packages': json['packages'] == null ? undefined : ((json['packages'] as Array<any>).map(MCPPackageFromJSON)),
        'registry': json['registry'] == null ? undefined : json['registry'],
        'remotes': json['remotes'] == null ? undefined : ((json['remotes'] as Array<any>).map(MCPRemoteFromJSON)),
        'repo': json['repo'] == null ? undefined : json['repo'],
        'site': json['site'] == null ? undefined : json['site'],
        'synced': json['synced'] == null ? undefined : json['synced'],
        'title': json['title'] == null ? undefined : json['title'],
        'transports': json['transports'] == null ? undefined : json['transports'],
        'vendor': json['vendor'] == null ? undefined : json['vendor'],
        'version': json['version'] == null ? undefined : json['version'],
    };
}

export function MCPListingToJSON(json: any): MCPListing {
    return MCPListingToJSONTyped(json, false);
}

export function MCPListingToJSONTyped(value?: MCPListing | null, ignoreDiscriminator: boolean = false): any {
    if (value == null) {
        return value;
    }

    return {
        
        'description': value['description'],
        'featured': value['featured'],
        'hidden': value['hidden'],
        'id': value['id'],
        'logo': value['logo'],
        'name': value['name'],
        'official': value['official'],
        'packages': value['packages'] == null ? undefined : ((value['packages'] as Array<any>).map(MCPPackageToJSON)),
        'registry': value['registry'],
        'remotes': value['remotes'] == null ? undefined : ((value['remotes'] as Array<any>).map(MCPRemoteToJSON)),
        'repo': value['repo'],
        'site': value['site'],
        'synced': value['synced'],
        'title': value['title'],
        'transports': value['transports'],
        'vendor': value['vendor'],
        'version': value['version'],
    };
}

